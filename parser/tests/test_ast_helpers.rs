    ...
    match parse_haxe_file("test.hx", input, false) {
        Ok(haxe_file) => {
            if let TypeDeclaration::Class(class) = &haxe_file.declarations[0] {
                ...
                assert_eq!(
                    primary.unwrap().params.len(),
                    0,
                    "Primary constructor should have no params"
                );

                // Test get_non_constructor_fields
                let non_constructor_fields: Vec<_> = class.get_non_constructor_fields().collect();
                assert_eq!(
                    non_constructor_fields.len(),
                    3,
                    "Should have 3 non-constructor fields"
                );
    ...
    match parse_haxe_file("test.hx", input, false) {
        Ok(haxe_file) => {
            if let TypeDeclaration::Abstract(abstract_decl) = &haxe_file.declarations[0] {
                // Test has_constructor
                assert!(
                    abstract_decl.has_constructor(),
                    "Abstract should have constructor"
                );
        ...
        Ok(haxe_file) => {
            if let TypeDeclaration::Class(class) = &haxe_file.declarations[0] {
                ...
                assert!(
                    ...
                    "Class should not have constructor"
                );
                ...
                // Test get_primary_constructor
                assert!(
                    class.get_primary_constructor().is_none(),
                    "Should not find primary constructor"
                );
            } else {
                panic!("Expected class declaration");
            }
