...
"#;
    ...
    if let Some(parser::haxe_ast::TypeDeclaration::Abstract(abstract_decl)) = ast.declarations.first() {
        let forward_meta = abstract_decl.meta.iter().find(|m| m.name == "forward");
        assert!(forward_meta.is_some(), "Should have @:forward metadata");
        assert!(forward_meta.unwrap().params.is_empty(), "Should have no parameters");
    } else {
...
"#;
    ...
    if let Some(parser::haxe_ast::TypeDeclaration::Abstract(abstract_decl)) = ast.declarations.first() {
        ...
        let has_to_meta = abstract_decl.fields.iter().any(|f| {
            f.meta.iter().any(|m| m.name == "to")
        });
        let has_from_meta = abstract_decl.fields.iter().any(|f| {
            f.meta.iter().any(|m| m.name == "from")
        });
...
"#;
    ...
    if let Some(parser::haxe_ast::TypeDeclaration::Abstract(abstract_decl)) = ast.declarations.first() {
        let forward_metas: Vec<_> = abstract_decl.meta.iter().filter(|m| m.name == "forward").collect();
        assert_eq!(forward_metas.len(), 2, "Should have 2 @:forward metadata");
        
        // Check first @:forward
        assert_eq!(forward_metas[0].params.len(), 1, "First @:forward should have 1 parameter");
        ...
        assert_eq!(forward_metas[1].params.len(), 1, "Second @:forward should have 1 parameter");
