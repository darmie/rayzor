        ...
        Ok(file) => {
            ...
            if let TypeDeclaration::Class(class) = &file.declarations[0] {
                if let ClassFieldKind::Function(func) = &class.fields[0].kind {
                    let has_macro = class.fields[0].modifiers.iter()
...
"#;
    assert!(parse_haxe_file("test.hx", input1, false).is_ok(),
...
"#;
    assert!(parse_haxe_file("test.hx", input2, false).is_ok(),
...
"#;
    assert!(parse_haxe_file("test.hx", input3, false).is_ok(),
