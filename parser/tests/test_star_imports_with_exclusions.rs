use parser::{parse_haxe_file, haxe_ast::*};
...
"#;
    ...
    let import = &file.imports[0];
    assert_eq!(import.path, vec!["haxe", "io"]);
    assert_eq!(import.mode, ImportMode::WildcardWithExclusions(vec!["Bytes".to_string()]));
}
...
"#;
    ...
    let import = &file.imports[0];
    assert_eq!(import.path, vec!["haxe", "io"]);
    assert_eq!(import.mode, ImportMode::WildcardWithExclusions(vec![
        "Bytes".to_string(),
        "Input".to_string(),
        "Output".to_string()
    ]));
...
"#;
    ...
    assert_eq!(file.imports[0].mode, ImportMode::WildcardWithExclusions(vec!["Bytes".to_string()]));
    ...
    assert_eq!(file.imports[2].mode, ImportMode::WildcardWithExclusions(vec![
        "File".to_string(),
        "FileInput".to_string()
    ]));
}
...
"#;
    ...
    assert_eq!(file.imports[1].mode, ImportMode::WildcardWithExclusions(vec![
