...
pub extern "C" fn rayzor_vec_i32_with_capacity(capacity: usize) -> *mut VecI32 {
    let cap = if capacity == 0 { INITIAL_CAPACITY } else { capacity };
    let layout = Layout::new::<VecI32>();
...
pub extern "C" fn rayzor_vec_i32_push(vec: *mut VecI32, value: i32) {
    ...
    unsafe {
        ...
        if v.len >= v.cap {
            ...
            let new_ptr = realloc(v.ptr as *mut u8, old_layout, new_cap * mem::size_of::<i32>());
            if new_ptr.is_null() {
...
pub extern "C" fn rayzor_vec_i32_sort_by(
    ...
    if vec.is_null() || compare_fn.is_null() { return; }
    unsafe {
        let v = &mut *vec;
        if v.len <= 1 { return; }
...
pub extern "C" fn rayzor_vec_i64_push(vec: *mut VecI64, value: i64) {
    ...
    unsafe {
        ...
        if v.len >= v.cap {
            ...
            let new_ptr = realloc(v.ptr as *mut u8, old_layout, new_cap * mem::size_of::<i64>());
            if new_ptr.is_null() { panic!("Vec<i64> allocation failed"); }
...
pub extern "C" fn rayzor_vec_f64_push(vec: *mut VecF64, value: f64) {
    ...
    unsafe {
        ...
        if v.len >= v.cap {
            ...
            let new_ptr = realloc(v.ptr as *mut u8, old_layout, new_cap * mem::size_of::<f64>());
            if new_ptr.is_null() { panic!("Vec<f64> allocation failed"); }
...
pub extern "C" fn rayzor_vec_ptr_push(vec: *mut VecPtr, value: *mut u8) {
    ...
    unsafe {
        ...
        if v.len >= v.cap {
            ...
            let new_ptr = realloc(v.ptr as *mut u8, old_layout, new_cap * mem::size_of::<*mut u8>());
            if new_ptr.is_null() { panic!("Vec<ptr> allocation failed"); }
...
pub extern "C" fn rayzor_vec_ptr_pop(vec: *mut VecPtr) -> *mut u8 {
    ...
    unsafe {
        let v = &mut *vec;
        if v.len == 0 { return ptr::null_mut(); }
        v.len -= 1;
        *v.ptr.add(v.len)
    }
}
...
pub extern "C" fn rayzor_vec_ptr_get(vec: *const VecPtr, index: usize) -> *mut u8 {
    ...
    unsafe {
        let v = &*vec;
        if index >= v.len { return ptr::null_mut(); }
        *v.ptr.add(index)
    }
}
...
pub extern "C" fn rayzor_vec_bool_push(vec: *mut VecBool, value: bool) {
    ...
    unsafe {
        ...
        if v.len >= v.cap {
            ...
            if new_ptr.is_null() { panic!("Vec<bool> allocation failed"); }
            v.ptr = new_ptr;
            v.cap = new_cap;
        }
...
mod tests {
    ...
    fn test_vec_bool_basic() {
        unsafe {
            ...
            assert_eq!(rayzor_vec_bool_get(vec, 0), true);
            assert_eq!(rayzor_vec_bool_get(vec, 1), false);
            assert_eq!(rayzor_vec_bool_get(vec, 2), true);
