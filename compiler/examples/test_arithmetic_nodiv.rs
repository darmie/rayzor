/// Test: Basic arithmetic WITHOUT division
///
/// This tests:
...
fn main() -> Result<(), String> {
    println!("\n=== Testing Arithmetic (No Division) ===\n");

    let mut unit = CompilationUnit::new(CompilationConfig::default());

    // Test arithmetic: (10 + 5) * 2 - 4 = 30 - 4 = 26
    let source = r#"
        package test;
    ...
    // Compile
    unit.load_stdlib()?;
    unit.add_file(source, "ArithmeticTest.hx")?;
    ...
    println!("\nExpected: 26 = (10 + 5) * 2 - 4 = 30 - 4");

    if result == 26 {
        println!("\n✅ TEST PASSED! Arithmetic (without division) works!");
        Ok(())
    } else {
