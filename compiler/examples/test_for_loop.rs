...
fn main() -> Result<(), String> {
    ...
    // Test traditional for loop using while
    // (Haxe for-in requires iterators which we may not have fully implemented yet)
    let source = r#"
        package test;
    ...
    unit.load_stdlib()?;
    unit.add_file(source, "ForTest.hx")?;
    ...
    // Verify result
    if result == 45 {
        println!("✅ TEST PASSED! For loop works correctly!");
        Ok(())
    } else {
        println!("❌ TEST FAILED! Expected 45, got {}", result);
