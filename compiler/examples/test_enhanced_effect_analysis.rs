...
use compiler::tast::{
    ...
    TypeTable, SymbolTable, SymbolId, TypeId, SourceLocation, StringInterner,
};
use std::rc::Rc;
...
fn test_async_function(analyzer: &mut EffectAnalyzer, string_interner: &Rc<RefCell<StringInterner>>) {
                ...
                expression: TypedExpression {
                    kind: TypedExpressionKind::Await {
                        expression: Box::new(TypedExpression {
                            kind: TypedExpressionKind::Variable { symbol_id: promise_var },
    ...
    assert_eq!(effects.async_kind, AsyncKind::Async, "Function should be detected as async");
...
fn test_memory_effects(analyzer: &mut EffectAnalyzer, string_interner: &Rc<RefCell<StringInterner>>) {
    ...
    assert!(!effects.is_pure, "Function with assignments should not be pure");
    assert!(effects.memory_effects.accesses_global_state, "Function should have memory effects");
    
    println!("✅ Memory effects analysis: PASSED");
    println!("  → Is pure: {}", effects.is_pure);
    println!("  → Accesses global state: {}", effects.memory_effects.accesses_global_state);
    println!("");
}

fn test_exception_effects(analyzer: &mut EffectAnalyzer, string_interner: &Rc<RefCell<StringInterner>>) {
...
fn test_pure_function(analyzer: &mut EffectAnalyzer, string_interner: &Rc<RefCell<StringInterner>>) {
            ...
            TypedStatement::Return {
                value: Some(TypedExpression {
                        kind: TypedExpressionKind::BinaryOp {
                            ...
                            right: Box::new(TypedExpression {
                                kind: TypedExpressionKind::Variable { symbol_id: y_symbol },
                                expr_type: TypeId::from_raw(1),
                                usage: VariableUsage::Copy,
                                lifetime_id: compiler::tast::LifetimeId::from_raw(0),
    ...
    assert_eq!(effects.async_kind, AsyncKind::Sync, "Pure function should be synchronous");
...
fn test_complex_effects(analyzer: &mut EffectAnalyzer, string_interner: &Rc<RefCell<StringInterner>>) {
                            ...
                            expression: Box::new(TypedExpression {
                                kind: TypedExpressionKind::FunctionCall {
                                    function: Box::new(TypedExpression {
                                        kind: TypedExpressionKind::Variable { symbol_id: SymbolId::from_raw(300) },
    ...
    assert_eq!(effects.async_kind, AsyncKind::Async, "Function should be async due to await");
    assert!(effects.can_throw, "Function should throw due to throw statement");
    assert!(!effects.is_pure, "Function with multiple effects should not be pure");
    assert!(effects.memory_effects.accesses_global_state, "Function should have memory effects");
    ...
    println!("  → Can throw: {}", effects.can_throw);
    println!("  → Is pure: {}", effects.is_pure);
    println!("  → Memory effects: {:?}", effects.memory_effects.accesses_global_state);
    println!("");
}