...
fn main() -> Result<(), String> {
    ...
    unit.load_stdlib()?;
    unit.add_file(source, "WhileTest.hx")?;
    ...
    let (main_func_id, main_func) = test_module.functions.iter()
    ...
    // Verify result
    if result == 15 {
        println!("✅ TEST PASSED! While loop works correctly!");
        Ok(())
    } else {
        println!("❌ TEST FAILED! Expected 15, got {}", result);
