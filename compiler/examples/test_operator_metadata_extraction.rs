...
fn main() -> Result<(), String> {
    ...
    println!("Found Counter abstract with {} methods\n", counter_abstract.methods.len());
    ...
    let add_method = counter_abstract.methods.iter()
    ...
    if !op_expr.contains("Add") && !op_expr.contains("+") {
        return Err(format!("❌ FAILED: Expected 'Add' or '+' operator, got: {}", op_expr));
    }

    println!("✅ TEST PASSED: Operator metadata correctly extracted!");
    println!("  - add() has operator: {}", op_expr);

    let multiply_method = counter_abstract.methods.iter()
    ...
    if !mul_expr.contains("Mul") && !mul_expr.contains("*") {
        return Err(format!("❌ FAILED: Expected 'Mul' or '*' operator, got: {}", mul_expr));
    }

    println!("  - multiply() has operator: {}", mul_expr);

    let negate_method = counter_abstract.methods.iter()
    ...
    if !neg_expr.contains("Neg") && !neg_expr.contains("-") {
        return Err(format!("❌ FAILED: Expected 'Neg' or '-' operator, got: {}", neg_expr));
    }
