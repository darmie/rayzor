...
use compiler::tast::{
    node::{TypedFunction, TypedStatement, TypedExpression, TypedExpressionKind, 
           LiteralValue, FunctionEffects, VariableUsage, ExpressionMetadata, FunctionMetadata},
    symbols::{Mutability, Visibility},
    TypeTable, SymbolTable, SymbolId, TypeId, SourceLocation, StringInterner, ScopeId,
};
...
impl TestResults {
    ...
    fn print_summary(&self) {
        ...
        for category in &self.test_categories {
            let pass_rate = if category.tests_run > 0 {
                (category.tests_passed * 100) / category.tests_run
            } else { 0 };
...
fn test_complex_control_flow(results: &mut TestResults) {
    ...
    results.record_test("Complex Control Flow", test1_passed, 
                       if !test1_passed { Some("Nested loop break/continue analysis failed".to_string()) } else { None });
    
    // Test 2: Complex conditional expressions
    let test2_passed = test_complex_conditional_expressions();
    results.record_test("Complex Control Flow", test2_passed,
                       if !test2_passed { Some("Complex conditional analysis failed".to_string()) } else { None });
    
    // Test 3: Short-circuit evaluation
    let test3_passed = test_short_circuit_evaluation();
    results.record_test("Complex Control Flow", test3_passed,
                       if !test3_passed { Some("Short-circuit evaluation analysis failed".to_string()) } else { None });
    
    // Test 4: Switch statement analysis
    let test4_passed = test_switch_statement_analysis();
    results.record_test("Complex Control Flow", test4_passed,
                       if !test4_passed { Some("Switch statement analysis failed".to_string()) } else { None });
...
fn test_advanced_null_safety(results: &mut TestResults) {
    ...
    results.record_test("Advanced Null Safety", test1_passed,
                       if !test1_passed { Some("Nullable generics analysis failed".to_string()) } else { None });
    
    // Test 2: Complex null state merging
    let test2_passed = test_complex_null_state_merging();
    results.record_test("Advanced Null Safety", test2_passed,
                       if !test2_passed { Some("Complex null state merging failed".to_string()) } else { None });
    
    // Test 3: Null safety in nested expressions
    let test3_passed = test_null_safety_nested_expressions();
    results.record_test("Advanced Null Safety", test3_passed,
                       if !test3_passed { Some("Nested expression null safety failed".to_string()) } else { None });
...
fn test_resource_management(results: &mut TestResults) {
    ...
    results.record_test("Resource Management", test1_passed,
                       if !test1_passed { Some("File handle tracking failed".to_string()) } else { None });
    
    // Test 2: Exception-safe cleanup
    let test2_passed = test_exception_safe_cleanup();
    results.record_test("Resource Management", test2_passed,
                       if !test2_passed { Some("Exception-safe cleanup analysis failed".to_string()) } else { None });
    
    // Test 3: RAII pattern validation
    let test3_passed = test_raii_pattern_validation();
    results.record_test("Resource Management", test3_passed,
                       if !test3_passed { Some("RAII pattern validation failed".to_string()) } else { None });
...
fn test_error_handling_robustness(results: &mut TestResults) {
    ...
    results.record_test("Error Handling", test1_passed,
                       if !test1_passed { Some("Malformed CFG handling failed".to_string()) } else { None });
    
    // Test 2: Circular reference detection
    let test2_passed = test_circular_reference_detection();
    results.record_test("Error Handling", test2_passed,
                       if !test2_passed { Some("Circular reference detection failed".to_string()) } else { None });
    
    // Test 3: Memory pressure scenarios
    let test3_passed = test_memory_pressure_scenarios();
    results.record_test("Error Handling", test3_passed,
                       if !test3_passed { Some("Memory pressure handling failed".to_string()) } else { None });
...
fn test_performance_stress(results: &mut TestResults) {
    ...
    results.record_test("Performance & Stress", test1_passed,
                       if !test1_passed { Some("Large function analysis performance issues".to_string()) } else { None });
    
    // Test 2: Deep nesting scenarios
    let test2_passed = test_deep_nesting_performance();
    results.record_test("Performance & Stress", test2_passed,
                       if !test2_passed { Some("Deep nesting performance issues".to_string()) } else { None });
    
    // Test 3: Memory usage validation
    let test3_passed = test_memory_usage_validation();
    results.record_test("Performance & Stress", test3_passed,
                       if !test3_passed { Some("Excessive memory usage detected".to_string()) } else { None });
...
fn test_integration_scenarios(results: &mut TestResults) {
    ...
    results.record_test("Integration", test1_passed,
                       if !test1_passed { Some("Cross-analysis integration failed".to_string()) } else { None });
    
    // Test 2: Incremental analysis
    let test2_passed = test_incremental_analysis();
    results.record_test("Integration", test2_passed,
                       if !test2_passed { Some("Incremental analysis failed".to_string()) } else { None });
    
    // Test 3: Multi-file analysis
    let test3_passed = test_multi_file_analysis();
    results.record_test("Integration", test3_passed,
                       if !test3_passed { Some("Multi-file analysis failed".to_string()) } else { None });
...
fn test_nested_loops_with_break_continue() -> bool {
    ...
    let success = results.metrics.functions_analyzed > 0 && 
                  results.metrics.blocks_processed >= 4; // Expect multiple blocks for nested loops
    
    if success {
        println!("    ✅ Nested loop analysis successful");
        println!("       Functions analyzed: {}, Blocks processed: {}", 
...
fn test_complex_conditional_expressions() -> bool {
    ...
    let func_name = string_interner.borrow_mut().intern("complexConditionalTest");
...
fn test_large_function_analysis() -> bool {
    ...
    if success {
        println!("    ✅ Large function analysis completed in {}ms", duration.as_millis());
        println!("       Functions analyzed: {}, Blocks processed: {}", 
                results.metrics.functions_analyzed, results.metrics.blocks_processed);
    } else {
        println!("    ❌ Large function analysis failed");
        println!("       Duration: {}ms (expected < 1000ms)", duration.as_millis());
        println!("       Functions analyzed: {}, Blocks processed: {}", 
                results.metrics.functions_analyzed, results.metrics.blocks_processed);
...
fn test_deep_nesting_performance() -> bool {
    ...
    if success {
        println!("    ✅ Deep nesting analysis completed in {}ms", duration.as_millis());
        println!("       Functions analyzed: {}, Blocks processed: {}", 
                results.metrics.functions_analyzed, results.metrics.blocks_processed);
    } else {
        println!("    ❌ Deep nesting analysis failed");
        println!("       Duration: {}ms (expected < 500ms)", duration.as_millis());
        println!("       Functions analyzed: {}, Blocks processed: {}", 
                results.metrics.functions_analyzed, results.metrics.blocks_processed);
        if results.metrics.functions_analyzed == 0 {
            println!("       Issue: CFG construction likely failed");
...
fn create_nested_loop_function(func_name: compiler::tast::InternedString, i_var: SymbolId, j_var: SymbolId) -> TypedFunction {
    TypedFunction {
                        ...
                        TypedStatement::While {
                            condition: TypedExpression {
                                kind: TypedExpressionKind::BinaryOp {
                                    ...
                                    right: Box::new(TypedExpression {
                                        kind: TypedExpressionKind::Literal { value: LiteralValue::Int(5) },
                                        expr_type: TypeId::from_raw(1),
                                        usage: VariableUsage::Copy,
                                        lifetime_id: compiler::tast::LifetimeId::from_raw(0),
                                        source_location: SourceLocation::new(0, 4, 1, 1),
            ...
            TypedStatement::If {
                ...
                then_branch: Box::new(TypedStatement::Block {
                    statements: vec![
                        ...
                        TypedStatement::Assignment {
                            target: TypedExpression {
                                kind: TypedExpressionKind::Variable { symbol_id: result_var },
                                expr_type: TypeId::from_raw(1),
                                usage: VariableUsage::Copy,
                                lifetime_id: compiler::tast::LifetimeId::from_raw(0),
                                source_location: SourceLocation::new(0, 5, 1, 1),
...
fn create_large_function(func_name: compiler::tast::InternedString, statement_count: usize) -> TypedFunction {
    ...
    for i in 0..statement_count {
        statements.push(TypedStatement::VarDeclaration {
            ...
            var_type: TypeId::from_raw(1),
            initializer: Some(TypedExpression {
                kind: TypedExpressionKind::Literal { value: LiteralValue::Int(i as i64) },
                expr_type: TypeId::from_raw(1),
                usage: VariableUsage::Copy,
                lifetime_id: compiler::tast::LifetimeId::from_raw(0),
...
fn create_deeply_nested_function(func_name: compiler::tast::InternedString, nesting_depth: usize) -> TypedFunction {
    ...
    fn create_nested_if(depth: usize, current_scope: u32) -> TypedStatement {
        if depth == 0 {
            ...
            TypedStatement::Assignment {
                target: TypedExpression {
                    kind: TypedExpressionKind::Variable { symbol_id: SymbolId::from_raw(200) },
                    expr_type: TypeId::from_raw(1),
                    usage: VariableUsage::Copy,
                    lifetime_id: compiler::tast::LifetimeId::from_raw(0),
                ...
                value: TypedExpression {
                    kind: TypedExpressionKind::Literal { value: LiteralValue::Int(depth as i64) },
                    expr_type: TypeId::from_raw(1),
                    usage: VariableUsage::Copy,
                    lifetime_id: compiler::tast::LifetimeId::from_raw(0),
                    source_location: SourceLocation::new(0, depth as u32 + 2, 1, 1),
    ...
    TypedFunction {
        ...
        body: vec![
            ...
            TypedStatement::Return {
                value: Some(TypedExpression {
                    kind: TypedExpressionKind::Variable { symbol_id: depth_var },
                    expr_type: TypeId::from_raw(1),
                    usage: VariableUsage::Copy,
                    lifetime_id: compiler::tast::LifetimeId::from_raw(0),
