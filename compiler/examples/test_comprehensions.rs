...
fn main() {
    ...
    let test_cases = vec![
        ...
        ("Array comprehension with condition", "[for (i in 0...10) if (i % 2 == 0) i]"),
        ("Nested array comprehension", "[for (i in 0...3) for (j in 0...3) i + j]"),
        
        // Basic map comprehensions
        ("Basic map comprehension", "[for (i in 0...5) i => i * i]"),
        ("Map comprehension with string keys", "[for (i in 0...3) \"key\" + i => i * 2]"),
        
        // Key-value iteration
        ("Key-value iteration", "[for (k => v in someMap) if (v > 5) k => v * 2]"),
        ("Key-value filtering", "[for (key => value in data) if (value != null) key => value.toString()]"),
    ];
    ...
    match parse_haxe_file("test.hx", &full_code, false) {
        Ok(haxe_file) => {
            ...
            if let Some(class) = haxe_file.declarations.first() {
                ...
                println!("  → Successfully parsed file with {} declaration(s)", haxe_file.declarations.len());
                Ok(Expr {
...
fn print_ast_info(ast: &Expr) {
    match &ast.kind {
        ExprKind::ArrayComprehension { for_parts, expr } => {
            println!("  → Array comprehension with {} for clause(s)", for_parts.len());
            println!("  → Expression: {:?}", expr.kind);
        }
        ExprKind::MapComprehension { for_parts, key, value } => {
            println!("  → Map comprehension with {} for clause(s)", for_parts.len());
...
fn test_advanced_features() {
    let advanced_tests = vec![
        ("Multiple conditions", "[for (i in 0...20) if (i % 2 == 0) if (i > 5) i]"),
        ("Complex expressions", "[for (i in 0...5) (i * i + 1) => Math.sqrt(i)]"),
        ("Nested comprehensions", "[for (row in matrix) [for (cell in row) cell * 2]]"),
        ("String operations", "[for (s in strings) if (s.length > 3) s.toUpperCase() => s.length]"),
    ];
            ...
            Ok(ast) => {
                match &ast.kind {
                    ExprKind::ArrayComprehension { for_parts, .. } => {
                        println!("✅ Parsed as array comprehension with {} for clause(s)", for_parts.len());
                    }
                    ExprKind::MapComprehension { for_parts, .. } => {
                        println!("✅ Parsed as map comprehension with {} for clause(s)", for_parts.len());
                    }
                    _ => {
