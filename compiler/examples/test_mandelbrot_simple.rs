...
fn main() {
    let source = std::fs::read_to_string(
        concat!(env!("CARGO_MANIFEST_DIR"), "/benchmarks/src/mandelbrot_simple.hx")
    ).expect("read");
    let symbols = get_runtime_symbols();

    println!("Compiling mandelbrot_simple...");
    let mut unit = CompilationUnit::new(CompilationConfig::fast());
    unit.load_stdlib().expect("stdlib");
    unit.add_file(&source, "mandelbrot_simple.hx").expect("parse");
