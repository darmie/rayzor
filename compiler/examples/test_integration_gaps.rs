...
use compiler::tast::{
    ...
    node::{TypedFunction, TypedStatement, TypedExpression, TypedExpressionKind, LiteralValue, 
           FunctionEffects, BinaryOperator, VariableUsage, ExpressionMetadata, FunctionMetadata},
...
fn main() {
    ...
    for (i, dead_code) in results.dead_code.iter().enumerate() {
        println!("  Dead Code #{}: {} at {}:{}", 
            i + 1, dead_code.message, dead_code.location.line, dead_code.location.column);
    }
    ...
    if !results.uninitialized_uses.is_empty() {
        ...
        if uninit_use.variable == x_symbol {
            ...
            println!("   Expected: {:?}, Got: {:?}", x_symbol, uninit_use.variable);
        }
    } else {
        ...
        println!("   Variables may not be properly registered or state propagation may be incomplete");
    }
