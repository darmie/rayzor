...
fn main() {
    ...
    println!("Core Functionality: {}", if coverage_percentage >= 70 { "✅ Covered" } else { "❌ Gaps" });
    println!("Error Handling: {}", if coverage_percentage >= 80 { "✅ Robust" } else { "⚠️ Needs work" });
    println!("Performance: {}", if coverage_percentage >= 85 { "✅ Validated" } else { "⚠️ Needs validation" });
    println!("Integration: {}", if coverage_percentage >= 90 { "✅ Solid" } else { "⚠️ Needs improvement" });
