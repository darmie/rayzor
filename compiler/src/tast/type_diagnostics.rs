...
use super::type_checker::{TypeCheckError, TypeErrorKind, AccessLevel};
use super::{TypeId, SymbolId, InternedString, SourceLocation, TypeTable, SymbolTable, StringInterner};
use super::node::BinaryOperator;
use crate::error_codes::format_error_code;
use diagnostics::{Diagnostic, DiagnosticBuilder, DiagnosticSeverity, SourceMap, SourceSpan, SourcePosition, FileId};
...
pub enum TypeErrorContext {
    Assignment { target_type: TypeId },
    BinaryOperation { operator: BinaryOperator, other_type: TypeId },
    UnaryOperation { operator: super::node::UnaryOperator },
    FunctionCall { param_index: usize, expected_type: TypeId },
    ReturnStatement { expected_type: TypeId },
    ArrayAccess,
    FieldAccess { field_name: InternedString },
    Initialization,
...
impl<'a> TypeSpecificSuggestions<'a> {
    pub fn new(type_table: &'a Rc<RefCell<TypeTable>>, string_interner: &'a StringInterner) -> Self {
        Self {
            type_table,
            string_interner,
    ...
    pub fn suggest_cast(&self, from_type: TypeId, to_type: TypeId) -> Option<String> {
        ...
        match (from_name.as_str(), to_name.as_str()) {
            ...
            ("Int", "Float") => Some("Automatic conversion available, or use explicit cast (value : Float)".to_string()),
            ("Float", "Int") => Some("Use Std.int(value) for truncation or Math.round() for rounding".to_string()),
    ...
    pub fn suggest_operation(&self, op: BinaryOperator, left_type: TypeId, right_type: TypeId) -> Option<String> {
        ...
        match op {
            BinaryOperator::Add => {
                ...
                } else if !self.std_types.is_numeric(&left_name) && !self.std_types.is_numeric(&right_name) {
    ...
    pub fn suggest_for_context(&self, from_type: TypeId, to_type: TypeId, context: &TypeErrorContext) -> Vec<String> {
        ...
        match context {
            ...
            TypeErrorContext::FunctionCall { param_index, .. } => {
                if let Some(cast_suggestion) = self.suggest_cast(from_type, to_type) {
                    suggestions.push(format!("Parameter {} expects {}: {}", 
                        param_index + 1, to_name, cast_suggestion));
                }
            }
            TypeErrorContext::ArrayAccess => {
                if from_name == "String" && to_name == "Int" {
                    suggestions.push("Array indices must be Int. Use Std.parseInt() to convert String to Int".to_string());
                } else if from_name == "Float" && to_name == "Int" {
                    suggestions.push("Array indices must be Int. Use Std.int() to convert Float to Int".to_string());
                } else if !self.std_types.is_numeric(&from_name) && to_name == "Int" {
                    suggestions.push(format!("Array indices must be Int. Convert {} to Int first", from_name));
                }
                
                // Suggestions for trying to index non-array types
                if to_name.starts_with("Array<") && !from_name.starts_with("Array<") {
                    if from_name == "String" {
                        suggestions.push("Strings can be indexed like arrays to get individual characters".to_string());
                    } else {
                        suggestions.push(format!("Cannot index {}. Use an Array or convert to Array first", from_name));
                    }
                }
            }
            TypeErrorContext::FieldAccess { field_name } => {
                ...
                if field_str == "length" && self.std_types.implements_tostring(&from_name) {
                    suggestions.push(format!("Convert {} to String first: Std.string(value).length", from_name));
                }
            }
            TypeErrorContext::UnaryOperation { operator } => {
                match operator {
                    super::node::UnaryOperator::Not => {
                        if from_name != "Bool" && to_name == "Bool" {
                            suggestions.push(format!("The ! operator requires a boolean. Convert {} to Bool first", from_name));
                        }
                    }
                    super::node::UnaryOperator::Neg => {
                        if !self.std_types.is_numeric(&from_name) && (to_name == "Int" || to_name == "Float") {
            ...
            TypeErrorContext::ConditionalExpression | TypeErrorContext::LoopCondition => {
                if from_name != "Bool" && to_name == "Bool" {
                    if from_name == "Int" || from_name == "Float" {
                        ...
                        suggestions.push("Check string is not empty: str != \"\" or str.length > 0".to_string());
            ...
            TypeErrorContext::SwitchPattern => {
                suggestions.push("Switch pattern must match the type being switched on".to_string());
                if let Some(cast_suggestion) = self.suggest_cast(from_type, to_type) {
...
impl<'a> TypeDiagnosticEmitter<'a> {
    ...
    pub fn get_suggestions(&self, from_type: TypeId, to_type: TypeId, context: &TypeErrorContext) -> Vec<String> {
        self.suggestion_generator.suggest_for_context(from_type, to_type, context)
    }
    ...
    pub fn emit_diagnostic(&self, error: TypeCheckError) -> Diagnostic {
        match error.kind {
            TypeErrorKind::TypeMismatch { expected, actual } => {
                self.emit_type_mismatch(error.location, expected, actual, &error.context, error.suggestion.as_deref())
            }
            ...
            // Remove these - they don't exist in TypeErrorKind
            
            TypeErrorKind::InvalidTypeArguments { base_type, expected_count, actual_count } => {
                self.emit_invalid_type_arguments(error.location, base_type, expected_count, actual_count)
            }
            
            TypeErrorKind::ConstraintViolation { type_param, constraint, violating_type } => {
                self.emit_constraint_violation(error.location, type_param, constraint, violating_type)
            }
            ...
            TypeErrorKind::SignatureMismatch { expected_params, actual_params, expected_return, actual_return } => {
                self.emit_signature_mismatch(error.location, expected_params, actual_params, expected_return, actual_return)
            }
            
            TypeErrorKind::AccessViolation { symbol_id, required_access } => {
            ...
            TypeErrorKind::InterfaceNotImplemented { interface_type, class_type, missing_method } => {
                self.emit_interface_not_implemented(error.location, interface_type, class_type, missing_method)
            }
            
            TypeErrorKind::MethodSignatureMismatch { expected, actual, method_name } => {
                self.emit_method_signature_mismatch(error.location, expected, actual, method_name, &error.context, error.suggestion.as_deref())
            }
            
            TypeErrorKind::MissingOverride { method_name, parent_class } => {
                self.emit_missing_override(error.location, method_name, parent_class, &error.context, error.suggestion.as_deref())
            }
            
            TypeErrorKind::InvalidOverride { method_name } => {
                self.emit_invalid_override(error.location, method_name, &error.context, error.suggestion.as_deref())
            }
            
            TypeErrorKind::StaticAccessFromInstance { member_name, class_name } => {
                self.emit_static_access_from_instance(error.location, member_name, class_name, &error.context, error.suggestion.as_deref())
            }
            
            TypeErrorKind::InstanceAccessFromStatic { member_name, class_name } => {
                self.emit_instance_access_from_static(error.location, member_name, class_name, &error.context, error.suggestion.as_deref())
            }
        }
    }
    ...
    fn emit_interface_not_implemented(
        ...
        let method_name = self.string_interner.get(missing_method).unwrap_or("<unknown>");
        let source_span = self.location_to_span(location);
        
        DiagnosticBuilder::error(
            format!("Class missing method '{}' required by interface", method_name),
            source_span.clone()
        )
        .code(format_error_code(1008))
        .label(source_span, format!("missing implementation of '{}'", method_name))
    ...
    fn emit_missing_override(
        ...
        let method_name_str = self.string_interner.get(method_name).unwrap_or("<unknown>");
        let parent_class_str = self.string_interner.get(parent_class).unwrap_or("<unknown>");
        let source_span = self.location_to_span(location);
        
        let mut builder = DiagnosticBuilder::error(
            format!("Method '{}' overrides parent method but missing 'override' modifier", method_name_str),
            source_span.clone()
        )
        .code(format_error_code(1010))
        .label(source_span, format!("overrides method from class '{}'", parent_class_str));
    ...
    fn emit_invalid_override(
        ...
        let mut builder = DiagnosticBuilder::error(
            format!("Method '{}' marked as 'override' but no parent method exists", method_name_str),
            source_span.clone()
        )
    ...
    fn emit_static_access_from_instance(
        ...
        let mut builder = DiagnosticBuilder::error(
            format!("Static member '{}' cannot be accessed through instance", member_name_str),
            source_span.clone()
        )
        ...
        if let Some(suggestion) = suggestion {
            ...
            builder = builder.help(format!("Access static members through the class: {}.{}", class_name_str, member_name_str));
        }
    ...
    fn emit_instance_access_from_static(
        ...
        let mut builder = DiagnosticBuilder::error(
            format!("Instance member '{}' cannot be accessed from static context", member_name_str),
            source_span.clone()
        )
        ...
        if let Some(suggestion) = suggestion {
            ...
            builder = builder.help(format!("Instance members require an instance of '{}'", class_name_str));
        }
    ...
    fn emit_type_mismatch(
        ...
        let mut builder = DiagnosticBuilder::error(
            format!("Type mismatch: expected `{}`, found `{}`", expected_name, actual_name),
            source_span.clone()
        )
        .code(format_error_code(1001))  // E1001: Type mismatch
        .label(source_span.clone(), format!("expected `{}`, found `{}`", expected_name, actual_name));
    ...
    fn emit_undefined_type(&self, location: SourceLocation, name: InternedString, context: &str) -> Diagnostic {
    ...
    fn emit_undefined_symbol(&self, location: SourceLocation, name: InternedString, context: &str) -> Diagnostic {
    ...
    fn emit_access_non_object(&self, location: SourceLocation, object_type: TypeId, field_name: InternedString, context: &str) -> Diagnostic {
        let type_name = self.format_type_name(object_type);
        let field_name_str = self.string_interner.get(field_name).unwrap_or("<unknown>");
        let source_span = self.location_to_span(location);
        
        DiagnosticBuilder::error(
            format!("Cannot access field `{}` on type `{}`", field_name_str, type_name),
            source_span.clone()
        )
        .code(format_error_code(1202))  // E1202: Field access on non-object
        .label(source_span, format!("field access on `{}`", type_name))
        .note(context)
        .help(format!("Only object types (classes, interfaces) have fields. `{}` is not an object type.", type_name))
        .build()
    }
    ...
    fn emit_invalid_type_arguments(
        ...
        .label(source_span, format!("expected {} type arguments", expected_count))
        .help(format!(
    ...
    fn emit_invalid_cast(&self, location: SourceLocation, from_type: TypeId, to_type: TypeId) -> Diagnostic {
        ...
        .label(source_span.clone(), format!("invalid cast from `{}` to `{}`", from_name, to_name))
    ...
    fn create_diagnostic_with_span(&self, 
        severity: &str,
        message: String, 
        span: &SourceSpan, 
        code: &str,
                ...
                match &type_info.kind {
                    ...
                    TypeKind::Function { params, return_type, .. } => {
                        let param_types: Vec<String> = params.iter()
                            .map(|&p| self.format_type_name(p))
                            .collect();
                        format!("({}) -> {}", param_types.join(", "), self.format_type_name(*return_type))
                    }
                    TypeKind::Array { element_type } => {
                    ...
                    TypeKind::Placeholder { name } => {
                        ...
                        self.string_interner.get(*name).unwrap_or("Unknown").to_string()
                    }
                    TypeKind::Unknown => "Unknown".to_string(),
                    _ => {
                        // For other types, fall back to a descriptive name
                        format!("{:?}", type_info.kind).split_whitespace().next().unwrap_or("Unknown").to_string()
                    }
                }
            }
            None => {
    ...
    fn format_function_signature(&self, params: &[TypeId], return_type: TypeId) -> String {
        ...
        format!("({}) -> {}", param_names.join(", "), self.format_type_name(return_type))
    }
