...
use super::type_checker::{
    ConstraintSet, ConstraintKind, TypeConstraint, ConstraintPriority, ConstraintValidation, ConstraintValidator
};
...
impl UnificationTable {
    ...
    pub fn assign(&mut self, var: UnificationVar, type_id: TypeId) -> Result<(), String> {
        ...
        if self.occurs_check(root, type_id) {
            ...
            return Err(format!("Occurs check failed: variable {:?} occurs in type {:?}", root, type_id));
        }
    ...
    pub fn assign_with_type_table(&mut self, var: UnificationVar, type_id: TypeId, type_table: &RefCell<TypeTable>) -> Result<(), String> {
        let root = self.find(var);
        ...
        if self.occurs_check_with_type_table(root, type_id, type_table) {
            self.stats.occurs_check_failures += 1;
            self.stats.cycles_detected += 1;
            return Err(format!("Cycle detected: variable {:?} occurs in type {:?}", root, type_id));
        }
    ...
    pub fn solve_constraints(&mut self, type_table: &RefCell<TypeTable>) -> Vec<UnificationResult> {
        ...
        while let Some(constraint) = self.active_constraints.pop_front() {
            ...
            match &result {
                UnificationResult::Success { additional_constraints, .. } => {
    ...
    fn occurs_check_recursive(&mut self, var: UnificationVar, type_id: TypeId, visited: &mut std::collections::HashSet<TypeId>) -> bool {
    ...
    fn occurs_check_with_type_table(&mut self, var: UnificationVar, type_id: TypeId, type_table: &RefCell<TypeTable>) -> bool {
    ...
    fn occurs_check_recursive_with_types(
        ...
        if let Some(type_obj) = type_table.borrow().get(type_id) {
            match &type_obj.kind {
                ...
                crate::tast::core::TypeKind::Abstract { underlying, type_args, .. } => {
                    underlying.map_or(false, |u| self.occurs_check_recursive_with_types(var, u, type_table, visited)) ||
                    type_args.iter().any(|&arg_type| 
                        self.occurs_check_recursive_with_types(var, arg_type, type_table, visited)
                    )
                }
                crate::tast::core::TypeKind::TypeAlias { target_type, type_args, .. } => {
    ...
    fn unify_equality(&mut self, left: TypeId, right: TypeId, _type_table: &RefCell<TypeTable>) -> UnificationResult {
    ...
    fn unify_subtype(&mut self, sub: TypeId, sup: TypeId, type_table: &RefCell<TypeTable>) -> UnificationResult {
        ...
        if ConstraintPropagationEngine::check_subtype_relationship(sub_resolved, sup_resolved, type_table) {
            UnificationResult::Success {
                substitutions: vec![],
                additional_constraints: vec![],
            }
            ...
            if self.union(sub_var, sup_var) {
                ...
                UnificationResult::Failure {
                    reason: format!("Type {:?} is not a subtype of {:?}", sub_resolved, sup_resolved),
    ...
    fn unify_structural(&mut self, left: TypeId, right: TypeId, type_table: &RefCell<TypeTable>) -> UnificationResult {
        let left_resolved = self.resolve_type(left);
        let right_resolved = self.resolve_type(right);
    ...
    fn unify_assignment(&mut self, source: TypeId, target: TypeId, _type_table: &RefCell<TypeTable>) -> UnificationResult {
    ...
    fn unify_type_structures(
        ...
        match (&left.kind, &right.kind) {
            ...
            (TypeKind::Array { element_type: e1 },
             TypeKind::Array { element_type: e2 }) => {
            ...
            (TypeKind::Optional { inner_type: i1 },
             TypeKind::Optional { inner_type: i2 }) => {
...
impl ConstraintPropagationEngine {
    ...
    fn register_default_rules(&mut self) {
        ...
        self.propagation_rules.push(PropagationRule {
            ...
            apply: |constraint, _constraint_set, type_table| {
                ...
                if let ConstraintKind::HasMethod { method_name: _, signature, is_static: _ } = &constraint.kind {
                    if let Some(sig_type) = type_table.borrow().get(*signature) {
                        if let TypeKind::Function { params, return_type, .. } = &sig_type.kind {
        ...
        self.propagation_rules.push(PropagationRule {
            ...
            apply: |constraint, _constraint_set, type_table| {
                ...
                let add_signature = type_table.borrow_mut().create_function_type(vec![self_type], self_type);
        ...
        self.propagation_rules.push(PropagationRule {
            ...
            apply: |constraint, _constraint_set, type_table| {
                ...
                let to_string_signature = type_table.borrow_mut().create_function_type(vec![], string_type);
        ...
        self.propagation_rules.push(PropagationRule {
            ...
            apply: |constraint, _constraint_set, type_table| {
                ...
                if let ConstraintKind::HasField { field_name: _, field_type, is_public: _ } = &constraint.kind {
        ...
        self.propagation_rules.push(PropagationRule {
            ...
            apply: |constraint, _constraint_set, type_table| {
                ...
                if let ConstraintKind::Callable { params, return_type } = &constraint.kind {
            ...
            match constraint {
                ...
                ConstraintKind::Comparable => {
                    match &type_obj.kind {
                        ...
                        TypeKind::Bool | TypeKind::Char => true,
                ...
                ConstraintKind::Copy => {
                    match &type_obj.kind {
                        ...
                        TypeKind::Class { .. } | TypeKind::Interface { .. } |
                        TypeKind::Function { .. } | TypeKind::Dynamic => false,
                ...
                ConstraintKind::Iterable { element_type } => {
                    match &type_obj.kind {
                        TypeKind::Array { element_type: array_elem } => {
                        ...
                        TypeKind::Map { key_type: _, value_type } => {
                        ...
                        TypeKind::String => {
                            ...
                            if let Some(expected_elem) = element_type {
                                ...
                                type_table.borrow().get(*expected_elem)
                ...
                ConstraintKind::HasMethod { method_name, signature, is_static } => {
                    match &type_obj.kind {
                        TypeKind::Class { symbol_id, .. } | 
                ...
                ConstraintKind::HasField { field_name, field_type, is_public } => {
                    match &type_obj.kind {
                        TypeKind::Class { symbol_id, .. } |
                        TypeKind::Interface { symbol_id, .. } => {
                ...
                ConstraintKind::IndexAccess { key_type, value_type } => {
                    match &type_obj.kind {
                        TypeKind::Array { element_type } => {
                            // Arrays use Int keys
                             if !type_table.borrow().get(*key_type)
                        ...
                        TypeKind::Map { key_type: map_key, value_type: map_value } => {
                ...
                ConstraintKind::Callable { params, return_type } => {
                    match &type_obj.kind {
                        TypeKind::Function { params: fn_params, return_type: fn_return, .. } => {
                ...
                ConstraintKind::Custom { predicate_name, args } => {
    ...
    fn check_convertible(source: TypeId, target: TypeId, type_table: &RefCell<TypeTable>) -> bool {
        ...
        if let (Some(source_type), Some(target_type)) = (type_table.borrow().get(source), type_table.borrow().get(target)) {
    ...
    fn check_unifiable(left: TypeId, right: TypeId, type_table: &RefCell<TypeTable>) -> bool {
        ...
        if let (Some(left_type), Some(right_type)) = (type_table.borrow().get(left), type_table.borrow().get(right)) {
    ...
    fn check_subtype_relationship(subtype: TypeId, supertype: TypeId, type_table: &RefCell<TypeTable>) -> bool {
        if subtype == supertype {
            return true;
        }
        
        if let (Some(sub_type), Some(super_type)) = (type_table.borrow().get(subtype), type_table.borrow().get(supertype)) {
    ...
    fn check_interface_implementation(type_id: TypeId, interface_id: TypeId, type_table: &RefCell<TypeTable>) -> bool {
        if type_id == interface_id {
            return true;
        }
        
        if let (Some(type_obj), Some(interface_obj)) = (type_table.borrow().get(type_id), type_table.borrow().get(interface_id)) {
...
impl<'a> ConstraintSolver<'a> {
    ...
    pub fn solve(
        ...
        while iteration < self.config.max_iterations {
            ...
            if self.config.aggressive_propagation {
                let inferred = self.propagation_engine.propagate(&mut constraints, type_table);
            ...
            let made_progress = results.iter().any(|r| matches!(r, UnificationResult::Success { .. }));
    ...
    fn convert_to_unification_constraints(&self, constraint_set: &ConstraintSet) -> Vec<UnificationConstraint> {
        ...
        for constraint in constraint_set.constraints() {
            match &constraint.kind {
                ...
                ConstraintKind::Callable { params, return_type } => {
                    ...
                    let func_type = self.type_table.borrow_mut().create_function_type(
                        params.clone(),
                        return_type.clone()
                    );
                    unification_constraints.push(UnificationConstraint {
