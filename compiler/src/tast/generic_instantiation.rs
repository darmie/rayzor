...
use super::type_checker::{ConstraintSet, ConstraintKind, TypeConstraint, ConstraintPriority};
...
impl GenericInstantiator {
    ...
    pub fn instantiate(
        ...
        self.stats.max_recursion_depth_used = 
            self.stats.max_recursion_depth_used.max(self.instantiation_stack.len());
            ...
            Ok(instantiation_result) => {
                ...
                if !request.is_partial || self.config.cache_partial {
                    if self.instantiation_cache.len() < self.config.max_cache_size {
                        self.instantiation_cache.insert(request, instantiation_result.clone());
                    }
                }
            }
    ...
    pub fn can_instantiate(
        ...
        self.validate_instantiation_request(base_type, type_args, type_table).is_ok()
    }
    ...
    pub fn validate_instantiation_request(
        ...
        let base_type_obj = binding.get(base_type)
        ...
        let expected_param_count = match &base_type_obj.kind {
            TypeKind::Class { type_args: params, .. } |
            TypeKind::Interface { type_args: params, .. } |
            TypeKind::Enum { type_args: params, .. } |
            TypeKind::Abstract { type_args: params, .. } |
            TypeKind::TypeAlias { type_args: params, .. } => params.len(),
            _ => {
    ...
    fn instantiate_impl(
        ...
        self.validate_instantiation_request(
            request.base_type,
            &request.type_args,
            &type_table,
        )?;
    ...
    fn extract_type_parameters(
        ...
        match &base_type.kind {
            ...
            TypeKind::TypeAlias { type_args, .. } => {
                ...
                Ok(type_args.iter().map(|&param_id| TypeParameterInfo {
                    id: param_id,
    ...
    fn perform_substitution(
        ...
        let instantiated_type = type_table.borrow_mut().create_generic_instance(base_type, type_args.to_vec());
    ...
    fn update_memory_usage(&mut self) {
        ...
        let cache_size = self.instantiation_cache.len() * std::mem::size_of::<(InstantiationRequest, InstantiationResult)>();
        let active_size = self.active_instantiations.len() * std::mem::size_of::<InstantiationRequest>();
        let stack_size = self.instantiation_stack.len() * std::mem::size_of::<InstantiationRequest>();
...
impl FastInstantiationContext {
    ...
    pub fn instantiate_builtin(
        ...
        if let Some(base_type_obj) = type_table.borrow().get(base_type) {
            let instantiated_type = match &base_type_obj.kind {
                ...
                TypeKind::Map { .. } if type_args.len() == 2 => {
                    Some(type_table.borrow_mut().create_map_type(type_args[0], type_args[1]))
                }
                ...
                TypeKind::Class { type_args: params, .. } if params.len() == type_args.len() => {
                    Some(type_table.borrow_mut().create_generic_instance(base_type, type_args.clone()))
                }
                TypeKind::Interface { type_args: params, .. } if params.len() == type_args.len() => {
                    Some(type_table.borrow_mut().create_generic_instance(base_type, type_args.clone()))
                }
                _ => None,
            };
    ...
    fn validate_builtin_args(
        ...
        let is_valid = match type_table.borrow().get(base_type) {
            Some(base_type_obj) => {
                match &base_type_obj.kind {
                    super::core::TypeKind::Class { type_args: params, .. } => {
...
impl fmt::Display for InstantiationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            InstantiationError::ArityMismatch { expected, actual, .. } => {
                write!(f, "Type argument count mismatch: expected {}, got {}", expected, actual)
            }
