...
impl Type {
    ...
    pub fn is_named_type(&self) -> bool {
        matches!(self.kind, 
            ...
            TypeKind::Enum { .. } | TypeKind::Abstract { .. } |
            TypeKind::TypeAlias { .. }
        )
    }
...
impl TypeTable {
    ...
    /// Create a new type with source location
    pub fn create_type_with_location(&mut self, kind: TypeKind, location: SourceLocation) -> TypeId {
    ...
    fn intern_type_with_location(&mut self, kind: TypeKind, location: SourceLocation) -> TypeId {
        ...
        // Update indices
        self.kind_index.entry(kind_hash).or_insert_with(Vec::new).push(type_id);
        
        // Index by symbol if it's a named type
        if let Some(symbol_id) = static_type_ref.symbol_id() {
            self.symbol_index.entry(symbol_id).or_insert_with(Vec::new).push(type_id);
        }
    ...
    fn hash_type_kind(&self, kind: &TypeKind) -> u64 {
        ...
        match kind {
            TypeKind::Void | TypeKind::Bool | TypeKind::Int | TypeKind::Char |
            ...
            TypeKind::Enum { symbol_id, type_args } => {
                symbol_id.hash(&mut hasher);
                type_args.hash(&mut hasher);
            }
            TypeKind::Abstract { symbol_id, underlying, type_args } => {
                symbol_id.hash(&mut hasher);
                underlying.hash(&mut hasher);
                type_args.hash(&mut hasher);
            }
            TypeKind::TypeAlias { symbol_id, target_type, type_args } => {
                symbol_id.hash(&mut hasher);
            ...
            TypeKind::TypeParameter { symbol_id, constraints, variance } => {
                symbol_id.hash(&mut hasher);
            ...
            TypeKind::Reference { target_type, mutability, lifetime_id } => {
                target_type.hash(&mut hasher);
    ...
    pub fn create_function_type_with_effects(
        ...
        self.intern_type(TypeKind::Function { params, return_type, effects })
    }
    ...
    pub fn create_abstract_type(
        ...
        self.intern_type(TypeKind::Abstract { symbol_id, underlying, type_args })
    }
    ...
    pub fn stats(&self) -> TypeTableStats {
        ...
        for type_ref in &self.types {
            match &type_ref.kind {
                TypeKind::Void | TypeKind::Bool | TypeKind::Int | 
                TypeKind::Float | TypeKind::String => primitive_count += 1,
                
                TypeKind::Class { .. } | TypeKind::Interface { .. } | 
                TypeKind::Enum { .. } | TypeKind::Abstract { .. } |
                TypeKind::TypeAlias { .. } => named_type_count += 1,
                
                TypeKind::TypeParameter { .. } | TypeKind::GenericInstance { .. } => generic_count += 1,
    ...
    pub fn most_used_types(&self, limit: usize) -> Vec<(TypeId, u32)> {
        let mut usage_pairs: Vec<(TypeId, u32)> = self.usage_stats
            .iter()
...
mod tests {
    ...
    fn test_function_types() {
        ...
        if let TypeKind::Function { params, return_type, .. } = &function.kind {
