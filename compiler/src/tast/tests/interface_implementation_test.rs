...
mod interface_implementation_tests {
    ...
    fn test_missing_interface_method() {
        ...
        let has_missing_method_error = result.errors.iter().any(|e| 
            e.message.contains("missing method") && e.message.contains("draw")
        );
        assert!(has_missing_method_error, "Should detect missing interface method");
    }
    ...
    fn test_interface_method_wrong_signature() {
        ...
        assert!(has_signature_error, "Should detect method signature mismatch");
    }
    ...
    fn test_correct_interface_implementation() {
        ...
        let has_interface_error = result.errors.iter().any(|e| 
            e.message.contains("interface") || e.message.contains("missing method")
        );
        assert!(!has_interface_error, "Should not have interface errors for correct implementation");
    }
    ...
    fn test_multiple_interface_implementation() {
        ...
        let has_missing_jump = result.errors.iter().any(|e| 
            e.message.contains("jump") && e.message.contains("missing")
        );
        assert!(has_missing_jump, "Should detect missing jump method from IJumpable");
    }
    ...
    fn test_interface_extends_interface() {
        ...
        println!("Interface inheritance errors: {:?}", 
            result.errors.iter()
