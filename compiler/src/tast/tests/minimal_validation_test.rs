...
class Main {
    static function main() {
        ...
        assert!(!typed_file.using_statements.is_empty(), "Should have using statements");
        ...
        let container = typed_file.classes.iter()
            .find(|c| string_interner.get(c.name).unwrap() == "Container")
            .expect("Container class not found");
        
        assert!(!container.type_parameters.is_empty(), "Container should have type parameters");
        assert!(!container.fields.is_empty(), "Container should have fields");
        assert!(!container.methods.is_empty(), "Container should have methods");
