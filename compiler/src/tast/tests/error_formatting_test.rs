    ...
    public function test() {
        ...
        match parse_haxe_file_with_diagnostics("test_errors.hx", haxe_code) {
            Ok(parse_result) => {
                ...
                let error_output = formatter.format_diagnostics(&parse_result.diagnostics, &parse_result.source_map);
                println!("Parser diagnostics:\n{}", error_output);
                
                // Verify the error output contains expected formatting
                assert!(error_output.contains("error") || error_output.contains("warning"), 
                    "Error output should contain severity level");
                assert!(error_output.contains("test_errors.hx"), "Error output should contain filename");
                assert!(error_output.contains("-->"), "Error output should contain source location arrow");
                
                // The error formatter should show the problematic lines with context
                assert!(error_output.contains("var x = this.field"), "Error should show the line missing semicolon");
                assert!(error_output.contains("if (x > 10 {"), "Error should show the line with missing parenthesis");
            }
            Err(error_output) => {
                println!("Parser error output:\n{}", error_output);
                
                // Also verify error formatting in case of complete failure
                assert!(error_output.contains("error"), "Error output should contain 'error' severity");
                assert!(error_output.contains("test_errors.hx"), "Error output should contain filename");
            }
        }
    }
    ...
    function method() {
        ...
        match parse_haxe_file_with_diagnostics("test_semicolon.hx", haxe_code) {
            Ok(parse_result) => {
                ...
                assert!(error_output.contains("test_semicolon.hx"), "Should show filename");
                assert!(error_output.contains("var x = 10"), "Should show the problematic line");
