...
mod override_validation_tests {
    ...
    fn test_missing_override_modifier() {
        ...
        let has_missing_override_error = result.errors.iter().any(|e| 
            e.message.contains("missing") && e.message.contains("override")
        );
        assert!(has_missing_override_error, "Should detect missing override modifier");
    }
    ...
    fn test_invalid_override_no_parent_method() {
        ...
        let has_invalid_override_error = result.errors.iter().any(|e| 
            e.message.contains("override") && e.message.contains("no parent method")
        );
    ...
    fn test_correct_override() {
        ...
        let has_override_error = result.errors.iter().any(|e| 
            e.message.contains("override")
        );
        assert!(!has_override_error, "Should not have override errors for correct usage");
    }
