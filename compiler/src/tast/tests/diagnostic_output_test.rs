...
class Cat extends Animal {
    ...
    override public function purr():String {
        ...
        assert!(result.errors.iter().any(|e| e.message.contains("E1010")), 
                "Should have missing override error (E1010)");
        assert!(result.errors.iter().any(|e| e.message.contains("E1011")), 
                "Should have invalid override error (E1011)");
        assert!(result.errors.iter().any(|e| e.message.contains("signature")), 
...
class Widget implements IDrawable implements IUpdatable {
    ...
    public function update(deltaTime:Float):Void {
        ...
        assert!(!result.errors.is_empty(), "Should have interface implementation errors");
    }
...
class Circle extends Shape implements IColorable {
    ...
    public function getColor():Int {
        ...
        // Group by error code
        let mut errors_by_code: std::collections::HashMap<String, Vec<&_>> = std::collections::HashMap::new();
        ...
        for error in &result.errors {
            if let Some(code_match) = error.message.find("[E") {
                if let Some(end) = error.message[code_match..].find(']') {
                    ...
                    errors_by_code.entry(code).or_insert_with(Vec::new).push(error);
                }
            }
        }
