...
mod tests {
    ...
    fn test_import_hx_types_available() {
        ...
        let iupdatable_symbol = symbol_table.create_interface_in_scope(iupdatable, ScopeId::first());
        let mathutils_symbol = symbol_table.create_class_in_scope(mathutils, ScopeId::first());
        
        scope_tree.get_scope_mut(ScopeId::first())
        ...
        // Lower the test file - it should have access to import.hx types
        {
            ...
            Ok(typed_file) => {
                ...
                assert!(!player_class.interfaces.is_empty(), "Should implement IUpdatable");
            }
            Err(error) => {
