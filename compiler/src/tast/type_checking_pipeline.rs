...
impl<'a> TypeCheckingPhase<'a> {
    ...
    pub fn new(
        ...
        let diagnostic_emitter = TypeDiagnosticEmitter::new(
            type_table,
            symbol_table,
            string_interner,
            source_map,
    ...
    fn check_type_declarations(&mut self, typed_file: &TypedFile) -> Result<(), String> {
        ...
        for class in &typed_file.classes {
            if !type_names.insert(&class.name) {
                self.emit_error(TypeCheckError {
                    ...
                    context: format!("Duplicate class definition: {}", self.get_string(class.name)),
        ...
        for interface in &typed_file.interfaces {
            if !type_names.insert(&interface.name) {
                self.emit_error(TypeCheckError {
                    kind: TypeErrorKind::UndefinedType { name: interface.name },
                    location: interface.source_location,
                    context: format!("Duplicate interface definition: {}", self.get_string(interface.name)),
    ...
    fn check_class(&mut self, class: &TypedClass) -> Result<(), String> {
        ...
        for &interface_type_id in &class.interfaces {
            if let Some(interface_symbol_id) = self.type_checker.symbol_table.get_symbol_from_type(interface_type_id) {
                self.verify_interface_implementation(class.symbol_id, interface_symbol_id, class.source_location)?;
            }
        }
    ...
    fn run_flow_analysis(&mut self, typed_file: &TypedFile) -> Result<(), String> {
        ...
        if let Some(ref mut flow_guard) = self.type_flow_guard {
            ...
            #[cfg(debug_assertions)]
            {
                ...
                eprintln!("  Functions analyzed: {}", results.metrics.functions_analyzed);
                eprintln!("  Blocks processed: {}", results.metrics.blocks_processed);
                eprintln!("  CFG construction time: {} μs", results.metrics.cfg_construction_time_us);
                eprintln!("  Variable analysis time: {} μs", results.metrics.variable_analysis_time_us);
                eprintln!("  Null safety time: {} μs", results.metrics.null_safety_time_us);
    ...
    fn emit_flow_safety_error(&mut self, error: &FlowSafetyError) {
        match error {
            FlowSafetyError::UninitializedVariable { variable, location } => {
                self.emit_error(TypeCheckError {
                    kind: TypeErrorKind::UndefinedType { 
                        name: self.string_interner.intern(&format!("uninitialized_var_{}", variable.as_raw()))
                    },
                    location: *location,
            ...
            FlowSafetyError::NullDereference { variable, location } => {
                self.emit_error(TypeCheckError {
                    kind: TypeErrorKind::UndefinedType { 
                        name: self.string_interner.intern(&format!("null_deref_{}", variable.as_raw()))
                    },
                    location: *location,
            ...
            FlowSafetyError::ResourceLeak { resource, location } => {
                self.emit_error(TypeCheckError {
                    kind: TypeErrorKind::UndefinedType { 
                        name: self.string_interner.intern(&format!("resource_leak_{}", resource.as_raw()))
                    },
                    location: *location,
    ...
    fn emit_flow_safety_warning(&mut self, warning: &FlowSafetyError) {
        match warning {
            FlowSafetyError::DeadCode { location } => {
                ...
                let span = SourceSpan::new(start_pos, end_pos, source_map::FileId::new(location.file_id as usize));
                let diagnostic = diagnostics::DiagnosticBuilder::hint(
                    "Dead code detected",
                    span,
                ).build();
    ...
    /// Check a field type
    fn check_field_type(&mut self, _symbol_id: SymbolId, type_id: TypeId, location: SourceLocation) -> Result<(), String> {
    ...
    fn check_method_signature(&mut self, method_name: InternedString, location: SourceLocation) -> Result<(), String> {
    ...
    fn check_method_implementation(&mut self, symbol_id: SymbolId, location: SourceLocation) -> Result<(), String> {
    ...
    fn check_method_body(&mut self, method: &TypedFunction, class_symbol_id: SymbolId) -> Result<(), String> {
    ...
    fn verify_interface_implementation(&mut self, class_id: SymbolId, interface_id: SymbolId, location: SourceLocation) -> Result<(), String> {
    ...
    fn verify_inheritance_signatures(&mut self, class: &TypedClass, super_type_id: TypeId) -> Result<(), String> {
        // Get the parent class symbol
        let super_symbol_id = if let Some(symbol_id) = self.type_checker.symbol_table.get_symbol_from_type(super_type_id) {
            symbol_id
        } else {
        ...
        let parent_class = if let Some(parent) = typed_file.classes.iter().find(|c| c.symbol_id == super_symbol_id) {
            parent
        } else {
    ...
    fn check_override_compatibility(&mut self, parent_method: &TypedFunction, child_method: &TypedFunction) -> Result<bool, String> {
        ...
        for (parent_param, child_param) in parent_method.parameters.iter().zip(child_method.parameters.iter()) {
            let param_compat = self.type_checker.check_compatibility(parent_param.param_type, child_param.param_type);
            match param_compat {
        ...
        let return_compat = self.type_checker.check_compatibility(child_method.return_type, parent_method.return_type);
        match return_compat {
            TypeCompatibility::Identical | TypeCompatibility::Assignable => {
    ...
    fn method_signatures_match(&mut self, interface_method: &TypedMethodSignature, class_method: &TypedFunction) -> Result<bool, String> {
        ...
        for (interface_param, class_param) in interface_method.parameters.iter().zip(class_method.parameters.iter()) {
            let compatibility = self.type_checker.check_compatibility(class_param.param_type, interface_param.param_type);
        ...
        let compatibility = self.type_checker.check_compatibility(class_method.return_type, interface_method.return_type);
        if matches!(compatibility, TypeCompatibility::Incompatible) {
            return Ok(false);
        }
        ...
        if let Some(type_info) = self.type_checker.type_table.borrow().get(type_id) {
            match &type_info.kind {
                ...
                TypeKind::Class { symbol_id, .. } => {
                    if let Some(symbol) = self.type_checker.symbol_table.get_symbol(*symbol_id) {
                        self.string_interner.get(symbol.name).unwrap_or("<unknown>").to_string()
                    } else {
        ...
        match &expr.kind {
            ...
            TypedExpressionKind::StaticFieldAccess { class_symbol, field_symbol } => {
                        ...
                        if let Some(symbol) = self.type_checker.symbol_table.get_symbol(*class_symbol) {
                            self.check_field_access(symbol.type_id, *field_symbol, expr.source_location, true)?;
                        }
                
                        // Static field access type checking completed
                    }
            TypedExpressionKind::StaticMethodCall { class_symbol, method_symbol, arguments, type_arguments: _ } => {
                            ...
                            if is_static_context {
                                ...
                                if let Some(symbol) = self.type_checker.symbol_table.get_symbol(*symbol_id) {
                                    ...
                                    if let Some(class_def) = self.find_class_by_symbol(class_symbol_id) {
                                        ...
                                        let matching_instance_field = class_def.fields.iter().find(|f| {
                                            f.name == symbol.name && !f.is_static
                                        });
            ...
            TypedExpressionKind::ArrayAccess { array, index } => {
                        ...
                        let index_compatibility = self.type_checker.check_compatibility(index_type, int_type);
                        ...
                        if !is_valid_indexable {
                            ...
                            self.emit_error(TypeCheckError {
                                ...
                                suggestion: Some("Only arrays, strings, and dynamic types can be indexed".to_string()),
                            });
                        }
                    }
                            ...
                            CastKind::Explicit => {
                                ...
                                if !self.is_valid_explicit_cast(source_type, *target_type) {
                                    self.emit_error(TypeCheckError {
                                        ...
                                        suggestion: Some("Check if the cast is supported or use safe conversion methods".to_string()),
                                    });
                                }
                            }
                            CastKind::Implicit => {
                                // Implicit casts should always be compatible
                                let compatibility = self.type_checker.check_compatibility(source_type, *target_type);
                                if matches!(compatibility, TypeCompatibility::Incompatible) {
                                    self.emit_enhanced_type_error(
                                        source_type,
                                        *target_type,
                                        ...
                                        &TypeErrorContext::Assignment { target_type: *target_type }
                                    );
                                }
                            }
                            CastKind::Checked => {
            ...
            TypedExpressionKind::UnaryOp { operand, operator } => {
                        ...
                        match operator {
                            super::node::UnaryOperator::Not => {
                                ...
                                let compatibility = self.type_checker.check_compatibility(operand_type, bool_type);
                                if matches!(compatibility, TypeCompatibility::Incompatible) {
                                    self.emit_enhanced_type_error(
                                        operand_type,
                                        bool_type,
                                        ...
                                        &TypeErrorContext::UnaryOperation { operator: *operator }
                                    );
                                }
                            }
                            super::node::UnaryOperator::Neg => {
                                ...
                                let compat_int = self.type_checker.check_compatibility(operand_type, int_type);
                                let compat_float = self.type_checker.check_compatibility(operand_type, float_type);
                        
                                let is_numeric = matches!(compat_int, TypeCompatibility::Identical | TypeCompatibility::Assignable) ||
                                               matches!(compat_float, TypeCompatibility::Identical | TypeCompatibility::Assignable);
            ...
            TypedExpressionKind::Conditional { condition, then_expr, else_expr } => {
                        ...
                        let compatibility = self.type_checker.check_compatibility(condition_type, bool_type);
                        if matches!(compatibility, TypeCompatibility::Incompatible) {
                            self.emit_enhanced_type_error(
                                condition_type,
                                bool_type,
            ...
            TypedExpressionKind::For { variable, iterable, body } => {
                        ...
                        if !is_iterable {
                            self.emit_error(TypeCheckError {
                                ...
                                suggestion: Some("Use an array, string, or other iterable type".to_string()),
                            });
                        }
            ...
            TypedExpressionKind::ObjectLiteral { fields } => {
                        ...
                        for field in fields {
                            ...
                            let field_name_str = self.string_interner.get(field.name)
                                .map(|s| s.to_string())
                            ...
                            if field_names.contains(&field.name) {
                                self.emit_error(TypeCheckError {
                                    ...
                                    context: format!("Duplicate field name '{}' in object literal", field_name_str),
                                    suggestion: Some("Field names in object literals must be unique".to_string()),
                                });
                            }
            ...
            TypedExpressionKind::ArrayLiteral { elements } => {
                        ...
                        if !elements.is_empty() {
                            ...
                            for (i, element) in elements.iter().enumerate().skip(1) {
                                ...
                                let compatibility = self.type_checker.check_compatibility(element_type, first_type);
                                if matches!(compatibility, TypeCompatibility::Incompatible) {
                                    self.emit_error(TypeCheckError {
                                        kind: TypeErrorKind::TypeMismatch {
                                            expected: first_type,
                                        ...
                                        suggestion: Some("All array elements must have compatible types".to_string()),
                                    });
                                }
                            }
                        }
            ...
            TypedExpressionKind::Return { value } => {
                        if let Some(return_expr) = value {
                            ...
                            if let Some(&expected_return) = self.expected_return_types.last() {
                                let compatibility = self.type_checker.check_compatibility(expr_type, expected_return);
                                if matches!(compatibility, TypeCompatibility::Incompatible) {
                                    self.emit_enhanced_type_error(
                                        expr_type,
                                        expected_return,
                                        ...
                                        &TypeErrorContext::ReturnStatement { expected_type: expected_return }
                                    );
                                }
                            }
                        }
                    }
            TypedExpressionKind::FunctionLiteral { parameters, body, return_type } => {
            ...
            TypedExpressionKind::New { class_type, arguments, type_arguments } => {
                        ...
                        if !type_arguments.is_empty() {
                            ...
                            if let Some(class_type_info) = type_table.get(*class_type) {
                                if let super::TypeKind::Class { symbol_id, type_args: class_type_params, .. } = &class_type_info.kind {
                                    let symbol_id = *symbol_id; // Copy the SymbolId
                                    // Get the class definition to check its type parameter constraints
                                    if let Some(_class_symbol) = self.type_checker.symbol_table.get_symbol(symbol_id) {
                                            ...
                                            for (i, type_arg) in type_arguments.iter().enumerate() {
                                                if i < class_def.type_parameters.len() {
                                                    ...
                                                    for constraint_type_id in &type_param.constraints {
                                                        if !self.validate_type_constraint(*type_arg, *constraint_type_id) {
                                                            violations.push((*type_arg, *constraint_type_id));
                                                        }
                                                    }
                                                }
                                            }
...
TypedExpressionKind::Await { expression, await_type } => todo!(),
    ...
    pub fn check_statement(&mut self, stmt: &TypedStatement) -> Result<(), String> {
        match stmt {
                    ...
                    if let Some(&expected_return) = self.expected_return_types.last() {
                        ...
                        if expected_return != void_type {
                            self.emit_error(TypeCheckError {
                                ...
                                suggestion: Some("Add a return value or change function return type to Void".to_string()),
                            });
                        }
                    }
                }
            }
            TypedStatement::VarDeclaration { var_type, initializer, source_location, .. } => {
            ...
            TypedStatement::Try { body, catch_clauses, finally_block, source_location } => {
                // Check try block
            ...
            TypedStatement::For { condition, body, source_location: _, .. } => {
            ...
            TypedStatement::ForIn { iterable, body, source_location: _, .. } => {
                // Check iterable type
                let iterable_type = self.check_expression(iterable)?;
            ...
            TypedStatement::Break { target_loop, source_location } => {
                ...
                if let Some(loop_symbol) = target_loop {
                    if self.type_checker.symbol_table.get_symbol(*loop_symbol).is_none() {
                        self.emit_error(TypeCheckError {
                            kind: TypeErrorKind::UndefinedSymbol { 
            ...
            TypedStatement::Continue { target_loop, source_location } => {
            ...
            TypedStatement::Block { statements, scope_id: _, source_location: _ } => {
                // Check all statements in the block
                for stmt in statements {
                    self.check_statement(stmt)?;
                }
    ...
    fn check_field_access(&mut self, object_type: TypeId, field_symbol: SymbolId, location: SourceLocation, is_static_access: bool) -> Result<(), String> {
        ...
        match &type_kind {
            super::TypeKind::Class { symbol_id: class_symbol, .. } => {
                // Check if the field belongs to this class
                if self.type_checker.symbol_table.get_symbol(field_symbol).is_some() {
                    // Verify that the field's scope matches the class or is accessible
                    self.check_field_accessibility(class_symbol, field_symbol, location, is_static_access)?;
                } else {
                    self.emit_error(TypeCheckError {
                        kind: TypeErrorKind::UndefinedType { 
            ...
            super::TypeKind::Interface { symbol_id: interface_symbol, .. } => {
                // Similar check for interfaces
                self.check_field_accessibility(interface_symbol, field_symbol, location, is_static_access)?;
            }
            super::TypeKind::Dynamic => {
            ...
            super::TypeKind::Anonymous { fields } => {
                ...
                if let Some(field_symbol_info) = self.type_checker.symbol_table.get_symbol(field_symbol) {
    ...
    fn check_field_accessibility(&mut self, class_symbol: &SymbolId, field_symbol: SymbolId, location: SourceLocation, is_static_access: bool) -> Result<(), String> {
        ...
        if is_static && !is_static_access {
            ...
            self.emit_error(TypeCheckError {
                ...
                location,
                context: "Static members should be accessed through the class, not an instance".to_string(),
                suggestion: Some(format!("Use {}.{} instead", 
                    self.string_interner.get(class_name).unwrap_or("<class>"),
            ...
            self.emit_error(TypeCheckError {
                ...
                location,
                context: "Instance members cannot be accessed from static context".to_string(),
                suggestion: Some("Create an instance of the class to access instance members".to_string()),
            });
            // Don't return early - continue checking
        }
    ...
    fn validate_field_visibility(&mut self, field_visibility: Visibility, target_class_symbol: SymbolId, field_symbol: SymbolId, location: SourceLocation) -> Result<(), String> {
        match field_visibility {
            ...
            Visibility::Private => {
                ...
                if let Some((_, current_class_symbol)) = self.current_method_context {
                    if current_class_symbol == target_class_symbol {
                        ...
                        let field_name_str = if let Some(symbol) = self.type_checker.symbol_table.get_symbol(field_symbol) {
                            self.get_string(symbol.name).to_string()
                        } else {
                            "<unknown_field>".to_string()
                        };
                        let target_class_name = if let Some(class_def) = self.find_class_by_symbol(target_class_symbol) {
                    ...
                    self.emit_error(TypeCheckError {
                        ...
                        location,
                        context: format!("Private field '{}' cannot be accessed from module level", field_name_str),
                        suggestion: Some("Make the field public to access from module level".to_string()),
                    });
                    Ok(())
                }
            },
            Visibility::Protected => {
                ...
                if let Some((_, current_class_symbol)) = self.current_method_context {
                    ...
                    self.emit_error(TypeCheckError {
                        ...
                        location,
                        context: format!("Protected field '{}' cannot be accessed from module level", field_name_str),
                        suggestion: Some("Make the field public to access from module level".to_string()),
                    });
                    Ok(())
                }
            ...
            Visibility::Internal => {
                ...
                self.validate_package_level_access(field_symbol, target_class_symbol, location, "internal field")
            },
        }
    }

    /// Validate package-level access for internal visibility
    fn validate_package_level_access(&mut self, target_symbol: SymbolId, target_class_symbol: SymbolId, location: SourceLocation, symbol_kind: &str) -> Result<(), String> {
        ...
        if let Some(ref mut validator) = self.package_access_validator {
            ...
            if let Some(file_name) = self.current_typed_file.and_then(|f| unsafe { (*f).metadata.file_name }) {
            ...
            let target_package = if let Some(target_symbol_info) = self.type_checker.symbol_table.get_symbol(target_symbol) {
                target_symbol_info.package_id
            } else if let Some(target_class_info) = self.type_checker.symbol_table.get_symbol(target_class_symbol) {
            ...
            let current_package = if let Some((_, current_class_symbol)) = self.current_method_context {
                // We're inside a class method - use class's package if available
                if let Some(current_class_info) = self.type_checker.symbol_table.get_symbol(current_class_symbol) {
            ...
            match (current_package, target_package) {
                ...
                _ => {
                    ...
                    let symbol_name = if let Some(symbol_info) = self.type_checker.symbol_table.get_symbol(target_symbol) {
    ...
    fn extract_package_from_file(&self, typed_file: &TypedFile) -> Option<super::namespace::PackageId> {
    ...
    fn find_class_by_symbol(&self, symbol_id: SymbolId) -> Option<&TypedClass> {
        if let Some(typed_file_ptr) = self.current_typed_file {
            ...
            // of the TypedFile reference in check_file, and we clear it after use
            let typed_file = unsafe { &*typed_file_ptr };
            typed_file.classes.iter().find(|class| class.symbol_id == symbol_id)
        } else {
            None
        }
    }
    ...
    fn check_method_static_access(&mut self, class_def: &TypedClass, method_symbol: SymbolId, location: SourceLocation, is_static_access: bool) -> Result<(), String> {
        ...
        if let Some(method) = class_def.methods.iter().find(|m| m.symbol_id == method_symbol) {
            if method.is_static && !is_static_access {
                ...
                self.emit_error(TypeCheckError {
                    ...
                    location,
                    context: "Instance methods cannot be accessed from static context".to_string(),
                    suggestion: Some("Create an instance of the class to access instance methods".to_string()),
                });
                // Don't return early - continue checking
            }
            
            // Add method visibility checking
            self.validate_method_visibility(method.visibility, class_def.symbol_id, method.symbol_id, location)?;
        }
    ...
    fn validate_method_visibility(&mut self, method_visibility: Visibility, target_class_symbol: SymbolId, method_symbol: SymbolId, location: SourceLocation) -> Result<(), String> {
        match method_visibility {
            ...
            Visibility::Private => {
                ...
                if let Some((_, current_class_symbol)) = self.current_method_context {
                    if current_class_symbol == target_class_symbol {
                        ...
                        // Different class - private access denied
                        let method_name_str = if let Some(symbol) = self.type_checker.symbol_table.get_symbol(method_symbol) {
                            self.get_string(symbol.name).to_string()
                        } else {
                    ...
                    self.emit_error(TypeCheckError {
                        ...
                        location,
                        context: format!("Private method '{}' cannot be accessed from module level", method_name_str),
                        suggestion: Some("Make the method public to access from module level".to_string()),
                    });
                    Ok(())
                }
            },
            Visibility::Protected => {
                ...
                if let Some((_, current_class_symbol)) = self.current_method_context {
                    ...
                    self.emit_error(TypeCheckError {
                        ...
                        location,
                        context: format!("Protected method '{}' cannot be accessed from module level", method_name_str),
                        suggestion: Some("Make the method public to access from module level".to_string()),
                    });
                    Ok(())
                }
            ...
            Visibility::Internal => {
                ...
                self.validate_package_level_access(method_symbol, target_class_symbol, location, "internal method")
            },
        }
    }
    ...
    fn is_comparable_type(&self, type_id: TypeId) -> bool {
        ...
        if let Some(type_info) = type_table.get(type_id) {
            match &type_info.kind {
                ...
                super::TypeKind::Int | super::TypeKind::Float | super::TypeKind::String | super::TypeKind::Bool => true,
    ...
    fn type_has_method(&self, _type_id: TypeId, _method_name: InternedString, _signature: TypeId) -> bool {
    ...
    fn type_has_field(&self, _type_id: TypeId, _field_name: InternedString, _field_type: TypeId) -> bool {
    ...
    fn constraint_type_to_string(&self, constraint_type_id: TypeId) -> String {
        ...
        self.get_type_name(constraint_type_id).unwrap_or_else(|| format!("Type#{}", constraint_type_id.as_raw()))
    }
    ...
    pub fn emit_enhanced_type_error(
        ...
        let suggestions = self.diagnostic_emitter.get_suggestions(
            actual_type, 
            expected_type, 
            error_context
        );
    ...
    pub fn emit_constraint_violation(
        ...
        let error = TypeCheckError {
            ...
            suggestion: Some("Ensure the type argument implements the required constraint".to_string()),
        };
        ...
        match (from_info, to_info) {
            (Some(from_type_info), Some(to_type_info)) => {
                ...
                match (&from_type_info.kind, &to_type_info.kind) {
                    ...
                    (TypeKind::Array { element_type: from_elem }, TypeKind::Array { element_type: to_elem }) => {
                        ...
                        *from_elem == *to_elem || 
                        self.are_both_basic_types(*from_elem, *to_elem)
                    }
                    
                    // Class hierarchy casts (upcast/downcast)
                    (TypeKind::Class { symbol_id: from_class, .. }, TypeKind::Class { symbol_id: to_class, .. }) => {
    ...
    fn are_both_basic_types(&self, type1: TypeId, type2: TypeId) -> bool {
        ...
        let is_basic_type = |type_id: TypeId| -> bool {
            if let Some(type_info) = type_table.get(type_id) {
                matches!(&type_info.kind, 
                    super::TypeKind::Int | super::TypeKind::Float | 
                    super::TypeKind::Bool | super::TypeKind::String |
                    super::TypeKind::Dynamic
                )
            } else {
                false
    ...
    fn check_method_overloads(&self, method_symbol: SymbolId, arg_types: &[TypeId], source_location: SourceLocation) -> bool {
                    ...
                    for method in &class.methods {
                        if method.symbol_id == method_symbol {
                            ...
                            for overload in &method.metadata.overload_signatures {
                                if self.check_signature_compatibility(&overload.parameter_types, arg_types) {
    ...
    fn validate_exception_type(&mut self, exception_type: TypeId, location: SourceLocation) -> Result<(), String> {
        let type_table = self.type_checker.type_table.borrow();
    ...
    fn validate_throwable_type(&mut self, throwable_type: TypeId, location: SourceLocation) -> Result<(), String> {
    ...
    fn validate_iterable_type(&mut self, iterable_type: TypeId, location: SourceLocation) -> Result<(), String> {
        let type_table = self.type_checker.type_table.borrow();
...
mod tests {
    use super::*;
    use crate::tast::{AstLowering, ScopeTree, ScopeId};
    ...
    fn test_type_checking_pipeline() {
        ...
        let mut typed_file = lowering.lower_file(&haxe_file).expect("Lowering should succeed");
        ...
        assert!(diagnostics.is_empty(), "Should have no type errors for valid code");
    }
    ...
    fn test_type_error_detection() {
        ...
        let mut diagnostics = Diagnostics::new();
        {
            ...
            type_checking_phase.emit_error(TypeCheckError {
                ...
                suggestion: Some("Change the type annotation to String or use an integer literal".to_string()),
            });
        }
...
class TryCatchTest {
    ...
    static function doSomething(): Void {}
        ...
        assert!(!result.errors.is_empty(), "Expected error for non-boolean catch filter");
        
        let has_filter_error = result.errors.iter().any(|e| 
            e.message.contains("filter") && e.message.contains("boolean")
        );
        assert!(has_filter_error, "Should have error about catch filter needing to be boolean");
    }
...
class WhileLoopTest {
    public static function main() {
        ...
        assert!(!result.errors.is_empty(), "Expected error for non-boolean while condition");
        
        let has_condition_error = result.errors.iter().any(|e| 
            e.message.contains("condition") && e.message.contains("boolean")
        );
        assert!(has_condition_error, "Should have error about while condition needing to be boolean");
    }
...
class ForLoopTest {
    public static function main() {
        ...
        let _has_condition_error = result.errors.iter().any(|e| 
            e.message.contains("condition") && e.message.contains("boolean")
        );
...
class ForInTest {
    public static function main() {
        ...
        assert!(!result.errors.is_empty(), "Expected error for non-iterable type in for-in");
        
        let has_iterable_error = result.errors.iter().any(|e| 
            e.message.contains("not iterable") || e.message.contains("Type is not iterable")
        );
        assert!(has_iterable_error, "Should have error about type not being iterable");
    }
...
class CustomException {
    ...
    public function new(msg: String) {
        ...
        assert!(result.errors.is_empty(), "Should not have errors for valid throw expressions");
    }
...
class ObjectLiteralTest {
    public static function main() {
        ...
        assert!(!result.errors.is_empty(), "Expected error for duplicate object field");
        
        let has_duplicate_error = result.errors.iter().any(|e| 
            e.message.contains("Duplicate field") || e.message.contains("duplicate")
        );
        assert!(has_duplicate_error, "Should have error about duplicate field in object literal");
    }
...
class MapLiteralTest {
    public static function main() {
        ...
        assert!(!result.errors.is_empty(), "Expected errors for map literal issues");
        ...
        assert!(has_key_mismatch || has_value_mismatch || has_duplicate_key, 
...
class OtherClass {
    public function testExternalAccess() {
        ...
        let has_private_access_error = result.errors.iter().any(|e| 
            e.message.contains("Private") && e.message.contains("privateField")
        );
...
class OtherClass {
    public function testMethodAccess() {
        ...
        let has_private_method_error = result.errors.iter().any(|e| 
            e.message.contains("Private") && e.message.contains("privateMethod")
        );
...
class SamePackageClass {
    public function testInternalAccess() {
        ...
        assert!(!has_internal_access_errors, "Internal access within same package should be allowed");
    }
...
class DifferentPackageClass {
    public function testCrossPackageAccess() {
        ...
        let has_internal_field_error = result.errors.iter().any(|e| 
            e.message.contains("Internal") && e.message.contains("internalField") && 
            e.message.contains("different package")
        );
        
        let has_internal_method_error = result.errors.iter().any(|e| 
            e.message.contains("Internal") && e.message.contains("internalMethod") && 
            e.message.contains("different package")
        );
...
class AnotherDefaultClass {
    public function testDefaultPackageAccess() {
        ...
        assert!(!has_internal_access_errors, "Internal access within default package should be allowed");
    }
}