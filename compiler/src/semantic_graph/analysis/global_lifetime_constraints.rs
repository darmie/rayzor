...
use crate::semantic_graph::analysis::lifetime_analyzer::{LifetimeAnalysisError, LifetimeConstraint};
use crate::semantic_graph::{CallGraph, CallSite, CallTarget};
use crate::tast::{SymbolId, LifetimeId, CallSiteId, TypeId, SourceLocation};
...
impl GlobalLifetimeConstraints {
    ...
    pub fn collect_function_signatures(&mut self, call_graph: &CallGraph) -> Result<(), LifetimeAnalysisError> {
    ...
    fn extract_function_signature(&self, function_id: SymbolId) -> Result<FunctionLifetimeSignature, LifetimeAnalysisError> {
    ...
    pub fn generate_call_site_constraints(&mut self, call_graph: &CallGraph) -> Result<(), LifetimeAnalysisError> {
        ...
        for call_site in call_graph.call_sites.values() {
            match &call_site.callee {
                ...
                CallTarget::Virtual { possible_targets, method_name, .. } => {
                    self.generate_virtual_call_constraints(call_site, possible_targets, method_name)?;
                }
                CallTarget::Dynamic { possible_targets, .. } => {
    ...
    fn generate_virtual_call_constraints(
        ...
        self.virtual_method_constraints.insert(call_site.caller, virtual_constraint);
    ...
    pub fn analyze_recursive_functions(&mut self, call_graph: &CallGraph) -> Result<(), LifetimeAnalysisError> {
    ...
    pub fn resolve_virtual_methods(&mut self, _call_graph: &CallGraph) -> Result<(), LifetimeAnalysisError> {
        // Unify constraints for virtual methods
        for virtual_constraint in self.virtual_method_constraints.values_mut() {
            if let Some(unified) = Self::unify_virtual_implementations(&virtual_constraint.implementations)? {
    ...
    pub fn build_constraint_graph(&mut self) -> Result<(), LifetimeAnalysisError> {
        ...
        for signature in self.function_signatures.values() {
            for &lifetime in &signature.parameter_lifetimes {
                self.constraint_graph.add_node(lifetime, NodeType::Parameter, signature.source_location);
            }
            
            if let Some(return_lifetime) = signature.return_lifetime {
                self.constraint_graph.add_node(return_lifetime, NodeType::Return, signature.source_location);
            }
        }
    ...
    fn tarjan_strongconnect(
        ...
        *index_counter += 1;
        ...
        let edges: Vec<_> = self.constraint_graph.edges.iter()
    ...
    fn solve_constraint_propagation(&mut self) -> Result<(), LifetimeAnalysisError> {
        ...
        for edge in &self.constraint_graph.edges {
            match edge.edge_type {
                EdgeType::Outlives => {
                    outlives_constraints.entry(edge.from).or_insert_with(HashSet::new).insert(edge.to);
                }
                EdgeType::Equal => {
                    equal_constraints.entry(edge.from).or_insert_with(HashSet::new).insert(edge.to);
                    equal_constraints.entry(edge.to).or_insert_with(HashSet::new).insert(edge.from);
                }
                EdgeType::Borrow => {
                ...
                for &outlived in outlives_set {
                    if let Some(transitive_outlives) = outlives_constraints.get(&outlived).cloned() {
                        for &transitive in &transitive_outlives {
                            if outlives_constraints.entry(lifetime).or_insert_with(HashSet::new).insert(transitive) {
                ...
                for &equal_lifetime in equal_set {
                    ...
                    if let Some(equal_outlives) = outlives_constraints.get(&equal_lifetime).cloned() {
                        for &outlived in &equal_outlives {
                            if outlives_constraints.entry(lifetime).or_insert_with(HashSet::new).insert(outlived) {
                                solution_changed = true;
                            }
                        }
                    }
                    ...
                    for (&other_lifetime, other_outlives) in &outlives_constraints.clone() {
                        if other_outlives.contains(&equal_lifetime) {
                            if outlives_constraints.entry(other_lifetime).or_insert_with(HashSet::new).insert(lifetime) {
                                solution_changed = true;
                            }
                        }
                    }
                ...
                for &lifetime_b in outlives_set_a {
                    if let Some(outlives_set_b) = outlives_constraints.get(&lifetime_b) {
                        if outlives_set_b.contains(&lifetime_a) {
                            ...
                            // They must be equal
                            if equal_constraints.entry(lifetime_a).or_insert_with(HashSet::new).insert(lifetime_b) {
                                solution_changed = true;
                            }
                            if equal_constraints.entry(lifetime_b).or_insert_with(HashSet::new).insert(lifetime_a) {
                                solution_changed = true;
                            }
                        }
                    }
            ...
            for &outlived in outlives_set {
                ...
                if let Some(equal_set) = equal_constraints.get(&lifetime) {
                    if equal_set.contains(&outlived) {
                        return Err(LifetimeAnalysisError::ContradictoryConstraints(lifetime, outlived));
                    }
                }
            }
        }
    ...
    fn solve_recursive_group(&self, group: &mut RecursiveConstraintGroup) -> Result<(), LifetimeAnalysisError> {
        let mut iteration = 0;
        ...
        while !converged && iteration < group.convergence_criteria.max_iterations {
            ...
            for constraint in &group.fixed_point_constraints {
                ...
                let new_solution = self.apply_constraint_equation(&constraint.equation, &previous_solution)?;
                
                // Check for convergence
                if !self.solutions_equal(&previous_solution, &new_solution, constraint.stability_threshold) {
                    converged = false;
                }
            }
        ...
        if !converged {
            return Err(LifetimeAnalysisError::RecursiveConstraintNonConvergence(group.functions.clone()));
        }
    ...
    fn evaluate_constraint_equation(&self, equation: &ConstraintEquation) -> Result<Vec<LifetimeId>, LifetimeAnalysisError> {
            ...
            match edge.edge_type {
                EdgeType::Outlives => {
                    ...
                    if self.has_path_in_constraint_graph(edge.to, edge.from) {
                        self.violations.push(GlobalLifetimeViolation::CrossFunctionUseAfterFree {
                ...
                EdgeType::Equal => {
                    ...
                    if self.has_outlives_relationship(edge.from, edge.to) || 
                       self.has_outlives_relationship(edge.to, edge.from) {
                        self.violations.push(GlobalLifetimeViolation::InvalidCrossFunctionBorrow {
    ...
    fn has_outlives_relationship(&self, from: LifetimeId, to: LifetimeId) -> bool {
        self.constraint_graph.edges.iter().any(|edge| {
            edge.from == from && edge.to == to && edge.edge_type == EdgeType::Outlives
        })
    }
    ...
    pub fn validate_solution(&mut self) -> Result<(), LifetimeAnalysisError> {
        ...
        if !self.violations.is_empty() {
            return Err(LifetimeAnalysisError::GlobalViolations(self.violations.clone()));
        }
...
impl LifetimeConstraintGraph {
    ...
    pub fn add_node(&mut self, lifetime: LifetimeId, node_type: NodeType, source_location: SourceLocation) {
    ...
    pub fn add_call_site_edges(&mut self, constraint: &CallSiteLifetimeConstraint) -> Result<(), LifetimeAnalysisError> {
...
impl fmt::Display for GlobalLifetimeViolation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            GlobalLifetimeViolation::CrossFunctionUseAfterFree { caller, callee, call_site, .. } => {
                write!(f, "Cross-function use-after-free: {:?} -> {:?} at call site {:?}", caller, callee, call_site)
            }
            GlobalLifetimeViolation::InvalidCrossFunctionBorrow { borrower, borrowed_from, call_site, .. } => {
                write!(f, "Invalid cross-function borrow: {:?} borrows from {:?} at call site {:?}", borrower, borrowed_from, call_site)
            }
            GlobalLifetimeViolation::RecursiveLifetimeExtension { recursive_group, .. } => {
                write!(f, "Recursive lifetime extension in functions: {:?}", recursive_group)
            }
            GlobalLifetimeViolation::VirtualMethodLifetimeMismatch { method_name, implementations, .. } => {
                write!(f, "Virtual method '{}' lifetime mismatch across implementations: {:?}", method_name, implementations)
            }
        }
    }
}
