...
use crate::tast::{
    DataFlowNodeId, SourceLocation, SsaVariableId, SymbolId, TypeId,
};
...
impl EscapeAnalyzer {
    ...
    pub fn analyze_function(
        ...
        for allocation in &allocations {
            ...
            if status == EscapeStatus::NoEscape {
                ...
                self.optimization_generator.stack_allocatable.insert(allocation.allocation_site);
            }
        ...
        if inlinable {
            self.optimization_generator.inlinable_functions.insert(context.function_id);
        }
        ...
        let total_lookups = self.allocation_tracker.cache_hits + self.allocation_tracker.cache_misses;
        ...
        Ok(EscapeAnalysisResults {
            ...
            escape_status,
            stack_allocatable,
            inlinable_functions: if inlinable { vec![context.function_id] } else { vec![] },
            optimization_hints,
    ...
    pub fn analyze_escapes(
        ...
        Ok(EscapeAnalysisResults {
            ...
            inlinable_functions: self.optimization_generator.inlinable_functions.iter().copied().collect(),
    ...
    fn find_allocation_sites(&mut self, dfg: &DataFlowGraph) -> Result<Vec<AllocationInfo>, EscapeAnalysisError> {
        ...
        for (node_id, node) in &dfg.nodes {
            ...
            if let Some(allocation_info) = self.detect_allocation_from_node(node)? {
                // Cache the result
                self.allocation_tracker.allocation_cache.insert(*node_id, allocation_info.clone());
    ...
    fn detect_allocation_from_node(&self, node: &DataFlowNode) -> Result<Option<AllocationInfo>, EscapeAnalysisError> {
        match &node.kind {
            DataFlowNodeKind::Allocation { allocation_type, size, allocation_kind } => {
            ...
            DataFlowNodeKind::Call { call_type: CallType::Constructor, .. } => {
    ...
    fn is_potentially_allocating_operation(&self, operator: &crate::tast::node::BinaryOperator) -> bool {
    ...
    fn analyze_node_for_escape(
        ...
        match &node.kind {
            ...
            DataFlowNodeKind::Store { address, .. } => {
                ...
                Ok(EscapeStatus::EscapesViaGlobal { global: SymbolId::from_raw(0) })
            }
    ...
    fn analyze_function_inlinability(&self, context: &FunctionAnalysisContext) -> Result<bool, EscapeAnalysisError> {
...
impl std::fmt::Display for EscapeStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::EscapesViaContainer { container } => write!(f, "Escapes via container {:?}", container),
...
impl std::fmt::Display for OptimizationHint {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::StackAllocation { allocation_site, estimated_size } => {
                write!(f, "Use stack allocation for {:?} (size: {} bytes)", allocation_site, estimated_size)
            }
            Self::InlineFunction { function, reason } => {
                write!(f, "Inline function {:?}: {}", function, reason)
            }
            Self::RemoveAllocation { allocation_site, reason } => {
                write!(f, "Remove allocation {:?}: {}", allocation_site, reason)
            }
            Self::CombineAllocations { allocations, combined_size } => {
                write!(f, "Combine {} allocations (total size: {} bytes)", allocations.len(), combined_size)
            }
        }
    }
}
