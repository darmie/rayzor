...
use crate::semantic_graph::analysis::lifetime_analyzer::{LifetimeAnalysisError, LifetimeAnalyzer, LifetimeConstraint, LifetimeViolation};
use crate::semantic_graph::analysis::ownership_analyzer::{OwnershipAnalyzer, OwnershipAnalysisError, OwnershipViolation};
use crate::semantic_graph::analysis::escape_analyzer::{EscapeAnalyzer, EscapeAnalysisError, EscapeAnalysisResults, OptimizationHint};
use crate::semantic_graph::analysis::deadcode_analyzer::{DeadCodeAnalyzer, DeadCodeAnalysisError, DeadCodeAnalysisResults, DeadCodeRegion};
pub use crate::semantic_graph::analysis::ownership_analyzer::FunctionAnalysisContext;
use crate::semantic_graph::{
    CallGraph, ControlFlowGraph, DataFlowGraph, DataFlowNode, DataFlowNodeKind, LifetimeId, OwnershipGraph, SemanticGraphs
};
use crate::tast::{SymbolId, BlockId, DataFlowNodeId, SourceLocation};
...
impl AnalysisEngine {
    ...
    pub fn analyze_function(
        ...
        let lifetime_results = self.lifetime_analyzer.analyze_function(&context).map_err(|err| AnalysisError::LifetimeAnalysisError(err))?;
        ...
        // Cache results
        self.analysis_results.function_results.insert(function_id, results.clone());
    ...
    fn run_lifetime_analysis(&mut self, graphs: &SemanticGraphs) -> Result<(), AnalysisError> {
        ...
        for (function_id, cfg) in &graphs.control_flow {
            if let Some(dfg) = graphs.dfg_for_function(*function_id) {
                ...
                let result = self.lifetime_analyzer.analyze_function(&function_context).map_err(|err| AnalysisError::LifetimeAnalysisError(err))?;
                self.analysis_results.function_lifetime_constraints.insert(*function_id, result.constraints);
    ...
    fn run_ownership_analysis(&mut self, graphs: &SemanticGraphs) -> Result<(), AnalysisError> {
        ...
        let global_violations = self.ownership_analyzer.check_ownership_violations(
        ...
        for (function_id, cfg) in &graphs.control_flow {
            if let Some(dfg) = graphs.dfg_for_function(*function_id) {
                ...
                let function_violations = self.ownership_analyzer.analyze_function(&function_context)?;
                
                if !function_violations.is_empty() {
                    self.analysis_results.function_ownership_violations
        ...
        for (function_id, dfg) in &graphs.data_flow {
            let move_violations = self.ownership_analyzer.check_move_semantics(
                dfg,
                &graphs.ownership_graph
            )?;
        ...
        self.diagnostics.extend(
            self.analysis_results.ownership_violations
                .iter()
    ...
    fn run_escape_analysis(&mut self, graphs: &SemanticGraphs) -> Result<(), AnalysisError> {
        ...
        let escape_info = self.escape_analyzer.analyze_escapes(
            &graphs.call_graph,
            &graphs.ownership_graph
        )?;
    ...
    fn run_deadcode_analysis(&mut self, graphs: &SemanticGraphs) -> Result<(), AnalysisError> {
        ...
        for (function_id, cfg) in &graphs.control_flow {
            let dead_blocks = self.deadcode_analyzer.find_dead_code(cfg, &graphs.call_graph)?;
            if !dead_blocks.is_empty() {
                self.analysis_results.dead_code_by_function.insert(*function_id, dead_blocks);
            }
        }
    ...
    fn validate_analysis_consistency(&self) -> Result<(), AnalysisError> {
        ...
        for (function_id, function_constraints) in &self.analysis_results.function_lifetime_constraints {
            if !self.analysis_results.global_lifetime_constraints.is_compatible(function_constraints) {
...
impl AnalysisResults {
    ...
    pub fn has_errors(&self) -> bool {
        ...
        self.function_ownership_violations.values().any(|v| !v.is_empty()) ||
    ...
    pub fn get_hir_hints(&self) -> HIRLoweringHints {
        HIRLoweringHints {
            ...
            dead_code_regions: self.dead_code_by_function.values().cloned().flatten().collect(),
...
impl AnalysisDiagnostic {
    pub fn severity(&self) -> DiagnosticSeverity {
        match self {
            ...
            Self::EscapeAnalysisWarning(_) | Self::DeadCodeWarning(_) => DiagnosticSeverity::Warning,
        }
    }
...
impl AnalysisMetrics {
    ...
    pub fn meets_performance_targets(&self) -> bool {
        self.total_time < Duration::from_millis(50) && 
        self.peak_memory_usage < 10 * 1024 * 1024 // 10MB
    }
}
...
impl std::fmt::Display for AnalysisError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::FunctionNotFound(id) => write!(f, "Function {} not found in semantic graphs", id.0),
            Self::LifetimeAnalysisError(msg) => write!(f, "Lifetime analysis error: {}", msg.to_string()),
...
impl OwnershipViolation {
    pub fn message(&self) -> String {
        match self {
            Self::UseAfterMove { variable, use_location, move_location, move_destination } => {
            ...
            Self::DoubleMove { variable, first_move, second_move } => {
                format!(
            ...
            Self::BorrowConflict { variable, mutable_borrow, conflicting_borrow, conflict_type } => {
                format!(
            ...
            Self::MoveOfBorrowedVariable { variable, move_location, active_borrows } => {
                let borrow_lines: Vec<String> = active_borrows.iter()
            ...
            Self::BorrowOutlivesOwner { borrowed_variable, borrower, borrow_location, owner_end_location } => {
                format!(
...
impl LifetimeViolation {
    pub fn message(&self) -> String { "Lifetime violation".to_string() }
}

impl EscapeWarning {
    pub fn message(&self) -> String { "Escape analysis warning".to_string() }
}

impl DeadCodeWarning {
    pub fn message(&self) -> String { "Dead code warning".to_string() }
}
