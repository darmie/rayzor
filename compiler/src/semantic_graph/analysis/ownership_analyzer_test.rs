...
use crate::semantic_graph::{
    CallGraph, ControlFlowGraph, DataFlowGraph, DataFlowNode, DataFlowNodeKind, OwnershipGraph, SemanticGraphs
};
use crate::semantic_graph::analysis::ownership_analyzer::{
    OwnershipAnalyzer, OwnershipAnalysisError, OwnershipViolation, FunctionAnalysisContext,
    BorrowConflictType, MoveOperation
};
use crate::semantic_graph::ownership_graph::{
    BorrowEdge, BorrowType, MoveEdge, MoveType, OwnershipKind, OwnershipNode
};
use crate::tast::{SymbolId, DataFlowNodeId, BorrowEdgeId, MoveEdgeId, SourceLocation, ScopeId, SsaVariableId, BlockId};
...
mod ownership_analysis_tests {
    ...
    fn test_complete_pipeline_simple_function() {
        ...
        assert!(violations.is_empty(), "Valid code should have no violations");
    ...
    fn test_lifetime_analysis_borrow_checking() {
        ...
        assert!(result_immutable.is_ok(), "Multiple immutable borrows should be valid");
        assert!(result_immutable.unwrap().is_empty(), "No violations expected for multiple immutable borrows");
        ...
        assert!(!violations_conflict.is_empty(), "Should detect borrow conflicts");
        
        // Check for specific borrow conflict violation
        let has_borrow_conflict = violations_conflict.iter().any(|v| {
            matches!(v, OwnershipViolation::BorrowConflict { 
        ...
        let has_outlive_violation = violations_outlive.iter().any(|v| {
            matches!(v, OwnershipViolation::BorrowOutlivesOwner { .. })
        });
    ...
    fn test_move_semantics_detection() {
        ...
        assert!(result_valid.unwrap().is_empty(), "Valid moves should have no violations");
        ...
        assert!(!violations_use_after_move.is_empty(), "Should detect use after move");
        
        let has_use_after_move = violations_use_after_move.iter().any(|v| {
            matches!(v, OwnershipViolation::UseAfterMove { .. })
        });
        assert!(has_use_after_move, "Should detect use-after-move violation");
        println!("Use after move detection: {} violations found", violations_use_after_move.len());
        ...
        let has_double_move = violations_double_move.iter().any(|v| {
            matches!(v, OwnershipViolation::DoubleMove { .. })
        });
       
        assert!(has_double_move, "Should detect double-move violation");
        println!("Double move detection: {} violations found", violations_double_move.len());
    }
    ...
    fn test_return_lifetime_validation() {
        ...
        assert!(result_param_return.is_ok(), "Return parameter reference should be valid");
        assert!(result_param_return.unwrap().is_empty(), "No violations for valid parameter return");
    ...
    fn test_ownership_state_tracking() {
        ...
        let violations = analyzer.check_ownership_violations(&graphs.ownership_graph, &graphs.call_graph);
        assert!(violations.is_ok(), "Ownership violation check should succeed");
        ...
        assert!(move_violations.is_ok(), "Move semantics check should succeed");
    ...
    fn test_error_message_quality() {
        ...
        if !violations.is_empty() {
            ...
            for violation in &violations {
                ...
                assert!(message.contains("variable"), "Message should mention variable: {}", message);
                
                // Should include location information
                assert!(message.contains("line"), "Message should include location: {}", message);
                
                // Should be descriptive
                assert!(message.len() > 20, "Message should be descriptive: {}", message);
    ...
    fn create_multiple_immutable_borrows() -> SemanticGraphs {
        ...
        semantic_graphs.ownership_graph.borrow_edges.insert(BorrowEdgeId::from_raw(1), borrow_edge_1);
        semantic_graphs.ownership_graph.borrow_edges.insert(BorrowEdgeId::from_raw(2), borrow_edge_2);
    ...
    fn create_mutable_immutable_conflict() -> SemanticGraphs {
        ...
        // Add ownership nodes
        semantic_graphs.ownership_graph.variables.insert(var_id, borrowed_node);
        semantic_graphs.ownership_graph.variables.insert(mutable_borrower, mutable_borrower_node);
        semantic_graphs.ownership_graph.variables.insert(immutable_borrower, immutable_borrower_node);
        ...
        semantic_graphs.ownership_graph.borrow_edges.insert(BorrowEdgeId::from_raw(1), mutable_borrow);
        semantic_graphs.ownership_graph.borrow_edges.insert(BorrowEdgeId::from_raw(2), immutable_borrow);
    ...
    fn create_borrow_outliving_owner() -> SemanticGraphs {
        ...
        // Add ownership nodes
        semantic_graphs.ownership_graph.variables.insert(owner_id, owner_node);
        semantic_graphs.ownership_graph.variables.insert(borrower_id, borrower_node);
        ...
        semantic_graphs.ownership_graph.borrow_edges.insert(BorrowEdgeId::from_raw(1), borrow_edge);
    ...
    fn create_valid_move_scenario() -> SemanticGraphs {
        ...
        semantic_graphs.ownership_graph.move_edges.insert(MoveEdgeId::from_raw(1), move_edge);
    ...
    fn create_use_after_move_scenario() -> SemanticGraphs {
        ...
        semantic_graphs.ownership_graph.variables.insert(moved_var, moved_ownership_node);
        ...
        semantic_graphs.data_flow.get_mut(&function_id).unwrap().ssa_variables.insert(ssa_var_id, ssa_var);
        ...
        semantic_graphs.data_flow.get_mut(&function_id).unwrap().nodes.insert(use_node_id, use_node);
    ...
    fn create_double_move_scenario() -> SemanticGraphs {
        ...
        semantic_graphs.ownership_graph.move_edges.insert(MoveEdgeId::from_raw(2), second_move);
    ...
    fn create_return_local_scenario() -> SemanticGraphs {
        ...
        semantic_graphs.data_flow.get_mut(&function_id).unwrap().nodes.insert(return_node_id, return_node);
    ...
    fn create_complex_function_graphs() -> SemanticGraphs {
        ...
        for i in 2..20 {
            ...
            semantic_graphs.ownership_graph.variables.insert(var_id, ownership_node);
            ...
            semantic_graphs.data_flow.get_mut(&function_id).unwrap().nodes.insert(node_id, node);
        }
    ...
    fn create_ownership_state_graphs() -> SemanticGraphs {
        ...
        semantic_graphs.ownership_graph.variables.insert(owned_var, owned_node);
        ...
        semantic_graphs.ownership_graph.variables.insert(moved_var, moved_node);
        ...
        semantic_graphs.ownership_graph.variables.insert(borrowed_var, borrowed_node);
    ...
    fn create_function_context(graphs: &SemanticGraphs) -> FunctionAnalysisContext {
        ...
        let function_id = *graphs.control_flow.keys().next().expect("No functions in graphs");
...
mod ownership_performance_tests {
    ...
    fn test_medium_codebase_performance() {
        ...
        println!("Medium codebase (100 functions) analyzed in {:?}", total_time);
    }
