...
mod integration_tests {
    ...
    use crate::semantic_graph::analysis::analysis_engine::{
        AnalysisEngine, AnalysisResults, FunctionAnalysisContext, AnalysisError
    };
    ...
    use crate::semantic_graph::{
        BasicBlock, BlockId, CallGraph, CallSite, CallType, ControlFlowGraph, 
        DataFlowGraph, DataFlowNode, DataFlowNodeKind, LifetimeId, OwnershipGraph,
        OwnershipNode, SemanticGraphs, Terminator
    };
    use crate::tast::collections::{new_id_map, new_id_set};
    use crate::tast::node::BinaryOperator;
    use crate::tast::{
        BlockId as TastBlockId, DataFlowNodeId, ScopeId, SourceLocation, 
    ...
    fn test_complete_pipeline_simple_function() {
        ...
        match result {
            Ok(results) => {
                ...
                println!("   Generated {} function-level constraints", results.function_lifetime_constraints.len());
                println!("   Found {} ownership violations", results.ownership_violations.len());
                println!("   Analysis time: {:?}", analysis_time);
                
                // Validate basic results structure
                assert!(results.function_lifetime_constraints.len() >= 0, "Should have function constraints map");
                
                // Performance validation
                assert!(analysis_time < Duration::from_millis(50), 
                        "Simple function analysis should complete in <50ms, took {:?}", analysis_time);
            }
            Err(analysis_error) => {
                ...
                panic!("Minimal valid function should pass analysis, but got: {:?}", analysis_error);
            }
        }
        
        // Validate metrics
        let metrics = engine.metrics();
        println!("   Performance metrics: functions_analyzed={}, meets_targets={}", 
                 metrics.functions_analyzed, metrics.meets_performance_targets());
    }
        ...
        // Test Case 1: Valid parameter return (should pass)
        {
            ...
            match result {
                Ok(analysis_result) => {
                    ...
                    println!("   Ownership violations: {}", analysis_result.ownership_violations.len());
                    // This should pass with no violations
                    assert_eq!(analysis_result.ownership_violations.len(), 0, 
                ...
                Err(analysis_error) => {
                    ...
                    panic!("Valid parameter return should pass, but got: {:?}", analysis_error);
                }
            }
        }
        ...
        // Test Case 2: Invalid borrow outliving owner (should fail)
        {
            ...
            match result {
                Ok(analysis_result) => {
                    ...
                    assert!(analysis_result.ownership_violations.len() > 0, 
                            "Borrow outliving owner should be detected as violation");
                    println!("   Correctly detected {} violations", analysis_result.ownership_violations.len());
                }
                Err(analysis_error) => {
        ...
        // Test Case 1: Return parameter reference (should pass)
        {
            ...
            match result {
                ...
                Err(analysis_error) => {
                    ...
                    panic!("Return parameter reference should pass, but got: {:?}", analysis_error);
                }
            }
        }
        ...
        // Test Case 2: Return local reference (should fail)
        {
            ...
            match result {
                Ok(analysis_result) => {
                    ...
                    assert!(has_violations, "Return local reference should be detected as violation");
    ...
    fn test_performance_targets() {
        ...
        for (name, function_count, target_time) in test_cases {
            println!("Testing {} with {} functions (target: {:?})", 
            ...
            match result {
                ...
                Err(_) => {
                    println!("  ⚠️  Analysis had constraint issues (acceptable for synthetic graphs)");
                }
            }
            ...
            let metrics = engine.metrics();
            assert!(metrics.peak_memory_usage < 20 * 1024 * 1024, 
    ...
    fn create_valid_parameter_return_graphs() -> SemanticGraphs {
        ...
        ownership_graph.add_variable(SymbolId::from_raw(100), TypeId::from_raw(1), ScopeId::from_raw(1));
    ...
    fn create_borrow_outliving_owner_graphs() -> SemanticGraphs {
        ...
        graphs.ownership_graph.add_variable(borrowed_var, TypeId::from_raw(1), ScopeId::from_raw(2));
        graphs.ownership_graph.add_variable(borrower_var, TypeId::from_raw(2), ScopeId::from_raw(1));
    ...
    fn create_use_after_move_graphs() -> SemanticGraphs {
        ...
        graphs.ownership_graph.add_variable(moved_var, TypeId::from_raw(3), ScopeId::from_raw(1));
        graphs.ownership_graph.add_variable(destination_var, TypeId::from_raw(3), ScopeId::from_raw(1));
