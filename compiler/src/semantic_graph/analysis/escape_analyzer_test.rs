...
mod escape_analysis_tests {
    use super::super::{
        escape_analyzer::*,
        ownership_analyzer::FunctionAnalysisContext,
    };
    use crate::semantic_graph::{
        CallGraph, ControlFlowGraph, DataFlowGraph, OwnershipGraph,
        DataFlowNode, DataFlowNodeKind, 
        CallSite, CallTarget, CallType,
        cfg::{BasicBlock, Terminator},
        dfg::{SsaVariable, ConstantValue, CallType as DfgCallType, NodeMetadata, LivenessInfo},
    };
    use crate::tast::{
        SymbolId, TypeId, BlockId, CallSiteId, SsaVariableId, DataFlowNodeId,
        SourceLocation, 
        node::{BinaryOperator},
    ...
    pub fn create_test_dfg_with_allocations() -> DataFlowGraph {
        ...
        // Add SSA variables
        dfg.ssa_variables.insert(SsaVariableId::from_raw(1), SsaVariable {
        ...
        dfg.ssa_variables.insert(SsaVariableId::from_raw(2), SsaVariable {
        ...
        dfg.ssa_variables.insert(SsaVariableId::from_raw(3), SsaVariable {
    ...
    pub fn create_test_call_graph() -> CallGraph {
        ...
        let constructor_call = CallSite::new(
            ...
            caller_func,
            CallTarget::Direct { function: constructor_func },
            CallType::Direct,
            BlockId::from_raw(1),
        ...
        let function_call = CallSite::new(
            ...
            caller_func,
            CallTarget::Direct { function: callee_func },
            CallType::Direct,
            BlockId::from_raw(2),
    ...
    fn test_allocation_site_detection() {
        ...
        let obj_alloc = result.allocation_sites.get(&DataFlowNodeId::from_raw(1))
    ...
    fn test_escape_via_return() {
        ...
        let obj_status = result.escape_status.get(&DataFlowNodeId::from_raw(1))
        ...
        let array_status = result.escape_status.get(&DataFlowNodeId::from_raw(2))
    ...
    fn test_optimization_hint_generation() {
        ...
        let stack_allocation_hints: Vec<_> = result.optimization_hints.iter()
            .filter(|hint| matches!(hint, OptimizationHint::StackAllocation { .. }))
            .collect();
        
        assert!(!stack_allocation_hints.is_empty());
        
        if let OptimizationHint::StackAllocation { allocation_site, estimated_size } = &stack_allocation_hints[0] {
    ...
    fn test_global_escape_analysis() {
        ...
        let result = analyzer.analyze_escapes(&call_graph, &ownership_graph)
...
mod escape_analysis_integration_tests {
    ...
    use crate::semantic_graph::{SemanticGraphs, CallGraph, OwnershipGraph};
...
mod escape_analysis_performance_tests {
    use crate::{semantic_graph::analysis::{analysis_engine::FunctionAnalysisContext, escape_analyzer::EscapeAnalyzer}, tast::SymbolId};
