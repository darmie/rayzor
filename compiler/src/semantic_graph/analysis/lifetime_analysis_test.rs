...
mod tests {
    ...
    use super::*;
    use crate::semantic_graph::analysis::lifetime_analyzer::{EqualityReason, LifetimeAnalyzer, LifetimeConstraint, LifetimeRegion, OutlivesReason};
    use crate::semantic_graph::analysis::lifetime_solver::{LifetimeConstraintSolver, LifetimeSolution, SolverConfig};
    use crate::semantic_graph::{BasicBlock, ControlFlowGraph, DataFlowGraph, DataFlowNode, DataFlowNodeKind, LifetimeId, OwnershipGraph, PhiIncoming, Terminator};
    use crate::tast::collections::new_id_set;
    use crate::tast::node::BinaryOperator;
    use crate::tast::{BlockId, DataFlowNodeId, ScopeId, SourceLocation, SsaVariableId, SymbolId, TypeId};
    ...
    fn test_constraint_solver_performance() {
        ...
        for size in test_sizes {
            ...
            assert!(solve_time.as_millis() < 100, "Constraint solving took too long: {:?}", solve_time);
    ...
    fn test_error_conversions() {
        use crate::semantic_graph::analysis::lifetime_analyzer::{LifetimeAnalysisError, ConstraintSolvingError};
    ...
    fn test_create_lifetime_regions() {
        ...
        let regions = analyzer.create_lifetime_regions(&cfg, function_id)
        ...
        let function_region = regions.iter().find(|r| r.parent == Some(LifetimeId::global()));
    ...
    fn test_binary_op_constraints() {
        ...
        let constraints = analyzer.generate_binary_op_constraints(left, right, &node)
    ...
    fn test_phi_constraints() {
        let analyzer = LifetimeAnalyzer::new();
        let incoming = vec![
            PhiIncoming { value: DataFlowNodeId::from_raw(1), predecessor: BlockId::from_raw(1) },
            PhiIncoming { value: DataFlowNodeId::from_raw(2), predecessor: BlockId::from_raw(2) },
        ];
        let node = create_test_phi_node();
        
        let constraints = analyzer.generate_phi_constraints(&incoming, &node)
    ...
    fn test_use_after_free_detection() {
        ...
        let violations = analyzer.check_use_after_free(&solution, &dfg)
    ...
    fn test_dangling_reference_detection() {
        ...
        let violations = analyzer.check_dangling_references(&solution, &ownership_graph)
    ...
    fn create_test_dfg() -> DataFlowGraph {
        ...
        dfg.ssa_variables.insert(SsaVariableId::from_raw(1), ssa_variable);
        dfg
    }
...
use crate::semantic_graph::analysis::lifetime_analyzer::{LifetimeConstraint, OutlivesReason, ConstraintSolvingError};
