...
use crate::tast::{
    BlockId, DataFlowNodeId, SourceLocation, SsaVariableId, SymbolId, TypeId,
};
...
impl DeadCodeAnalyzer {
    ...
    pub fn analyze_function(
        ...
        let total_lookups = self.reachability_analyzer.cache_hits + self.reachability_analyzer.cache_misses;
    ...
    fn find_unreachable_blocks_in_cfg(
        ...
        for (&block_id, _block) in &cfg.blocks {
            ...
            self.unreachable_block_detector.block_cache.insert(block_id, reachability);
    ...
    fn find_unused_variables(
        ...
        for (&ssa_var_id, ssa_var) in &dfg.ssa_variables {
            ...
            // Cache the result
            self.unused_variable_detector.usage_cache.insert(ssa_var_id, usage);
    ...
    fn is_debug_only_variable(&self, ssa_var: &crate::semantic_graph::dfg::SsaVariable) -> bool {
        ...
        let debug_patterns = [
            "debug", "log", "trace", "assert", "temp", "tmp", 
            "_debug", "_log", "_trace", "_test", "dbg"
        ];
    ...
    fn find_dead_stores(&mut self, dfg: &DataFlowGraph) -> Result<Vec<DeadCodeRegion>, DeadCodeAnalysisError> {
    ...
    fn find_unreachable_functions(&mut self, call_graph: &CallGraph) -> Result<Vec<SymbolId>, DeadCodeAnalysisError> {
        let mut unreachable = Vec::new();
        let mut visited = HashSet::new();
        let mut worklist = VecDeque::new();
    ...
    fn get_unreachability_reason(&self, block_id: BlockId, cfg: &ControlFlowGraph) -> Option<UnreachabilityReason> {
        // Check if block has any predecessors
        let has_predecessors = cfg.blocks.values().any(|block| block.successors.contains(&block_id));
...
impl DeadCodeAnalysisResults {
    ...
    pub fn get_dead_regions_by_type(&self) -> HashMap<String, Vec<&DeadCodeRegion>> {
        ...
        for region in &self.dead_code_regions {
            ...
            by_type.entry(type_name.to_string()).or_insert_with(Vec::new).push(region);
        }
...
impl std::fmt::Display for DeadCodeRegion {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnreachableBlock { block, function, reason, .. } => {
                write!(f, "Unreachable block {:?} in function {:?}: {:?}", block, function, reason)
            }
            Self::UnusedVariable { variable, suggested_action, .. } => {
                write!(f, "Unused variable {:?}: {}", variable, suggested_action)
            }
            Self::UnreachableFunction { function, call_graph_analysis, estimated_savings, .. } => {
                write!(f, "Unreachable function {:?}: {} (saves ~{} bytes)", function, call_graph_analysis, estimated_savings)
            }
            Self::DeadStore { variable, .. } => {
                write!(f, "Dead store to variable {:?}", variable)
            }
            Self::UnreachableAfterReturn { function, .. } => {
                write!(f, "Unreachable code after return in function {:?}", function)
            }
        }
    }
}
