...
use crate::semantic_graph::cfg::{ControlFlowGraph, BasicBlock};
...
impl DominanceTree {
    ...
    pub fn build(cfg: &ControlFlowGraph) -> Result<Self, DominanceError> {
        ...
        tree.stats.edges_processed = cfg.blocks.values()
        ...
        for (block_id, block) in &cfg.blocks {
            for &successor in &block.successors {
                if !cfg.blocks.contains_key(&successor) {
                    return Err(DominanceError::InvalidCFG(
                        format!("Block {:?} references non-existent successor {:?}", 
                               block_id, successor)
                    ));
                }
            }
            ...
            for &predecessor in &block.predecessors {
                if !cfg.blocks.contains_key(&predecessor) {
                    return Err(DominanceError::InvalidCFG(
                        format!("Block {:?} references non-existent predecessor {:?}", 
    ...
    fn compute_dfs_numbering(&mut self, cfg: &ControlFlowGraph) -> Result<(), DominanceError> {
        ...
        let unreachable_blocks: Vec<BlockId> = cfg.blocks.keys()
    ...
    fn build_dominance_tree(&mut self) {
        ...
        for (&child, &parent) in &self.idom {
            if child != parent { // Don't add entry as child of itself
        ...
        for &block in self.idom.keys() {
            ...
            while let Some(curr) = current {
                ...
                current = self.idom.get(&curr).copied()
            ...
            for &block_y in &block_info.successors {
                if !self.dominates(block_x, block_y) {
                    ...
                    while let Some(curr) = runner {
                        ...
                        runner = self.idom.get(&curr).copied()
    ...
    pub fn immediate_dominator(&self, block: BlockId) -> Option<BlockId> {
        self.idom.get(&block).copied()
            .filter(|&idom| idom != block) // Entry block's idom is itself
    }
    ...
    fn get_dfs_parent(&self, block: BlockId, cfg: &ControlFlowGraph) -> Result<BlockId, DominanceError> {
        ...
        predecessors.iter()
            ...
            .copied()
            .ok_or_else(|| DominanceError::AlgorithmError(
                format!("No DFS parent found for block {:?}", block)
            ))
    }
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        // DFS numbering
        size += self.dfs_preorder.len() * (std::mem::size_of::<BlockId>() + std::mem::size_of::<usize>());
        size += self.dfs_postorder.len() * (std::mem::size_of::<BlockId>() + std::mem::size_of::<usize>());
