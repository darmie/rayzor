...
impl CallGraph {
    ...
    pub fn add_function(&mut self, function_id: SymbolId) {
        ...
        self.direct_calls.entry(function_id).or_insert_with(Vec::new);
    }
    ...
    pub fn get_calls_from(&self, function: SymbolId) -> &[CallSiteId] {
        self.direct_calls.get(&function).map_or(&[], |v| v.as_slice())
    }
    ...
    pub fn is_recursive(&self, function: SymbolId) -> bool {
        ...
        self.recursion_info.mutually_recursive.iter()
        ...
        for mut scc in scc_components {
            ...
            if scc.has_cycles {
                if scc.functions.len() == 1 {
                    ...
                    self.recursion_info.directly_recursive.insert(scc.functions[0]);
                } else {
                    // Multi-function SCC with cycles = mutual recursion
                    self.recursion_info.mutually_recursive.push(scc.functions.clone());
                }
            }
            ...
            if let Some(call_site) = self.get_call_site(call_site_id) {
                for w in call_site.get_possible_callees() {
                    if !indices.contains_key(&w) {
                        ...
                        self.tarjan_strongconnect(w, index_counter, stack, indices, lowlinks, on_stack, sccs);
    ...
    pub fn update_statistics(&mut self) {
        ...
        self.statistics.recursive_function_count = 
            self.recursion_info.directly_recursive.len() +
            self.recursion_info.mutually_recursive.iter().map(|group| group.len()).sum::<usize>();
...
impl CallSite {
    ...
    pub fn get_possible_callees(&self) -> Vec<SymbolId> {
        match &self.callee {
            ...
            CallTarget::Virtual { possible_targets, .. } => possible_targets.clone(),
            CallTarget::Dynamic { possible_targets, .. } => possible_targets.clone(),
...
impl fmt::Display for CallTarget {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CallTarget::Virtual { method_name, receiver_type, possible_targets } => {
                write!(f, "virtual({}@{:?}, {} targets)", method_name, receiver_type, possible_targets.len())
            }
            CallTarget::Dynamic { possible_targets, .. } => {
                write!(f, "dynamic({} targets)", possible_targets.len())
            }
            CallTarget::External { function_name, module } => {
...
mod tests {
    ...
    fn test_recursion_detection() {
        ...
        assert!(call_graph.recursion_info.directly_recursive.contains(&func_a));
    }
