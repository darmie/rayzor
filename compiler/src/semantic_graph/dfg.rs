...
pub enum CanonicalExpression {
    ...
    BinaryOp { op: BinaryOperator, left: ValueNumber, right: ValueNumber },
    UnaryOp { op: UnaryOperator, operand: ValueNumber },
    FieldAccess { object: ValueNumber, field: SymbolId },
    ArrayAccess { array: ValueNumber, index: ValueNumber },
}
...
impl DataFlowGraph {
    ...
    pub fn add_node(&mut self, mut node: DataFlowNode) -> DataFlowNodeId {
        ...
        for &operand in &node.operands {
            ...
            self.def_use_chains.def_to_uses
                .entry(operand)
    ...
    pub fn nodes_in_block(&self, block_id: BlockId) -> &[DataFlowNodeId] {
        self.block_nodes.get(&block_id).map_or(&[], |v| v.as_slice())
    }
    ...
    pub fn get_uses(&self, def: DataFlowNodeId) -> &[DataFlowNodeId] {
        self.def_use_chains.def_to_uses.get(&def).map_or(&[], |v| v.as_slice())
    }
        ...
        while let Some(node_id) = worklist.pop_front() {
            if let Some(node) = self.nodes.remove(&node_id) {
                ...
                for &operand in &operands_to_check {
                    let should_add_to_worklist = if let Some(operand_node) = self.nodes.get_mut(&operand) {
    ...
    pub fn statistics(&self) -> DfgStatistics {
        ...
        let phi_node_count = self.nodes.values()
            .filter(|n| matches!(n.kind, DataFlowNodeKind::Phi { .. }))
            .count();
        let constant_count = self.nodes.values()
...
impl fmt::Display for DataFlowNodeKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            DataFlowNodeKind::Parameter { parameter_index, .. } => {
...
mod tests {
    ...
    fn test_node_addition() {
        ...
        let node = DataFlowNode {
            ...
            kind: DataFlowNodeKind::Constant { value: ConstantValue::Int(42) },
    ...
    fn test_dfg_statistics() {
        ...
        for i in 1..=5 {
            let node = DataFlowNode {
                ...
                kind: DataFlowNodeKind::Constant { value: ConstantValue::Int(i as i64) },
                value_type: TypeId::from_raw(1),
                source_location: SourceLocation::unknown(),
                operands: vec![],
                uses: new_id_set(),
