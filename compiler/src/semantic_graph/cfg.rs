...
impl ControlFlowGraph {
    ...
    pub fn validate_with_options(&self, strict_unreachable_check: bool) -> Result<(), CfgValidationError> {
    ...
    pub fn statistics(&self) -> CfgStatistics {
        ...
        let edge_count: usize = self.blocks.values()
            .map(|block| block.successors.len())
            .sum();
        let statement_count: usize = self.blocks.values()
            .map(|block| block.statements.len())
            .sum();
        
        let max_loop_depth = self.blocks.values()
...
impl fmt::Display for CfgValidationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CfgValidationError::InconsistentEdges { from, to, description } => {
                write!(f, "Inconsistent edge from {:?} to {:?}: {}", from, to, description)
            }
            CfgValidationError::InvalidSuccessor { block_id, successor_id } => {
                write!(f, "Block {:?} references invalid successor {:?}", block_id, successor_id)
            }
