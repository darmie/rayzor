...
use crate::tast::node::{TypedStatement, TypedExpression, TypedFunction};
...
impl TastCfgMapping {
    ...
    pub fn get_statements_in_block(&self, block_id: BlockId) -> &[StatementLocation] {
        self.block_to_statements.get(&block_id).map(|v| v.as_slice()).unwrap_or(&[])
    }
    ...
    pub fn get_ordered_statements(&self, block_id: BlockId) -> &[StatementLocation] {
        self.statement_order.get(&block_id).map(|v| v.as_slice()).unwrap_or(&[])
    }
...
impl TastCfgMapper {
    ...
    fn map_function_statements(&mut self, cfg: &ControlFlowGraph, function: &TypedFunction) {
        ...
        for &block_id in cfg.blocks.keys() {
            self.mapping.block_to_statements.insert(block_id, Vec::new());
    ...
    fn map_statement(&mut self, cfg: &ControlFlowGraph, statement: &TypedStatement, location: StatementLocation, depth: u32) {
        ...
        self.mapping.statement_locations.insert(location, statement.source_location());
        ...
        match statement {
            ...
            TypedStatement::If { condition: _, then_branch, else_branch, .. } => {
            ...
            TypedStatement::While { condition: _, body, .. } => {
            ...
            TypedStatement::For { init, condition: _, update: _, body, .. } => {
            ...
            TypedStatement::Block { statements, .. } => {
                ...
                for (index, stmt) in statements.iter().enumerate() {
                    let stmt_location = self.create_nested_location(location, depth + 1, index as u64);
            ...
            TypedStatement::Try { body, catch_clauses, finally_block, .. } => {
                ...
                for (index, catch_clause) in catch_clauses.iter().enumerate() {
                    ...
                    let catch_location = self.create_nested_location(location, depth + 1, index as u64 + 1);
                ...
                if let Some(finally_stmt) = finally_block {
                    ...
                    let finally_location = self.create_nested_location(location, depth + 1, catch_clauses.len() as u64 + 1);
            ...
            TypedStatement::Switch { cases, default_case, .. } => {
                ...
                for (index, case) in cases.iter().enumerate() {
                    ...
                    let case_location = self.create_nested_location(location, depth + 1, index as u64);
                ...
                if let Some(default_stmt) = default_case {
                    ...
                    let default_location = self.create_nested_location(location, depth + 1, cases.len() as u64);
            ...
            TypedStatement::PatternMatch { patterns, .. } => {
                ...
                for (index, pattern_case) in patterns.iter().enumerate() {
                    ...
                    let pattern_location = self.create_nested_location(location, depth + 1, index as u64);
            ...
            TypedStatement::MacroExpansion { expanded_statements, .. } => {
                ...
                for (index, stmt) in expanded_statements.iter().enumerate() {
                    let expanded_location = self.create_nested_location(location, depth + 1, index as u64);
                    self.map_statement(cfg, stmt, expanded_location, depth + 1);
                }
            }
            
            TypedStatement::ForIn { value_var: _, key_var: _, iterable: _, body, .. } => {
    ...
    fn map_statement_to_block(&mut self, location: StatementLocation, block_id: BlockId) {
        ...
        self.mapping.block_to_statements
            .entry(block_id)
            .or_insert_with(Vec::new)
            .push(location);
        
        // Add to ordered list
        self.mapping.statement_order
            .entry(block_id)
            .or_insert_with(Vec::new)
            .push(location);
    }
    ...
    fn create_nested_location(&mut self, parent: StatementLocation, depth: u32, index: u64) -> StatementLocation {
        let location = StatementLocation {
    ...
    fn find_else_block(&self, cfg: &ControlFlowGraph, current_block: BlockId) -> BlockId {
        ...
        if let Some(block) = cfg.get_block(current_block) {
            block.successors.get(1).copied().unwrap_or_else(|| 
                block.successors.first().copied().unwrap_or(current_block)
            )
        } else {
            current_block
    ...
    fn find_catch_block(&self, cfg: &ControlFlowGraph, try_block: BlockId, catch_index: usize) -> BlockId {
        // Simplified: find catch block based on index
        if let Some(block) = cfg.get_block(try_block) {
            block.successors.get(catch_index + 1).copied().unwrap_or(try_block)
        } else {
            try_block
        }
    }
    ...
    fn find_case_block(&self, cfg: &ControlFlowGraph, switch_block: BlockId, case_index: usize) -> BlockId {
        if let Some(block) = cfg.get_block(switch_block) {
            block.successors.get(case_index).copied().unwrap_or(switch_block)
        } else {
            switch_block
        }
    }
    ...
    fn find_pattern_block(&self, cfg: &ControlFlowGraph, match_block: BlockId, pattern_index: usize) -> BlockId {
        if let Some(block) = cfg.get_block(match_block) {
            block.successors.get(pattern_index).copied().unwrap_or(match_block)
        } else {
            match_block
        }
    }
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        let block_map_size = self.mapping.block_to_statements.len() * std::mem::size_of::<BlockId>() +
            self.mapping.block_to_statements.values()
...
mod tests {
    ...
    fn test_nested_statement_mapping() {
        ...
        assert_eq!(result.stats.successful_mappings, result.stats.statements_processed);
    }
}
