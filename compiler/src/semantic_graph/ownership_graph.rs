...
use super::{SourceLocation, SymbolId, SourceLocationTracker, SourceLocationTracking};
...
impl OwnershipGraph {
    ...
    pub fn add_variable(&mut self, variable: SymbolId, variable_type: TypeId, scope: ScopeId) -> &mut OwnershipNode {
    ...
    pub fn add_borrow(&mut self, 
                     borrower: SymbolId, 
                     borrowed: SymbolId, 
                     borrow_type: BorrowType,
                     borrow_scope: ScopeId,
                     location: SourceLocation) -> BorrowEdgeId {
        ...
        self.add_lifetime_constraint(LifetimeConstraint::Outlives {
            longer: self.get_variable_lifetime(borrowed).unwrap_or(LifetimeId::global()),
    ...
    pub fn get_ownership_kind(&self, variable: SymbolId) -> Option<OwnershipKind> {
        self.variables.get(&variable).map(|node| node.ownership_kind)
    }
    ...
    pub fn get_borrowers(&self, variable: SymbolId) -> Vec<SymbolId> {
        if let Some(node) = self.variables.get(&variable) {
            node.borrowed_by.iter()
                .filter_map(|&edge_id| {
                    self.borrow_edges.get(&edge_id).map(|edge| edge.borrower)
                })
                .collect()
        } else {
            Vec::new()
    ...
    pub fn get_borrowed_from(&self, variable: SymbolId) -> Vec<SymbolId> {
        if let Some(node) = self.variables.get(&variable) {
            node.borrows_from.iter()
                .filter_map(|&edge_id| {
                    self.borrow_edges.get(&edge_id).map(|edge| edge.borrowed)
                })
                .collect()
        } else {
            Vec::new()
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            OwnershipValidationError::InvalidLifetime { variable, lifetime } => {
                write!(f, "Invalid lifetime {:?} for variable {:?}", lifetime, variable)
            }
            OwnershipValidationError::InvalidBorrow { edge_id, variable } => {
                write!(f, "Invalid borrow edge {:?} references non-existent variable {:?}", edge_id, variable)
            }
            OwnershipValidationError::InvalidMove { edge_id, variable } => {
                write!(f, "Invalid move edge {:?} references non-existent variable {:?}", edge_id, variable)
            }
        }
    }
}
