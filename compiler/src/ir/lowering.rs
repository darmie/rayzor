...
use super::{
    IrBuilder, IrModule, IrType, IrId, IrBlockId, IrSourceLocation,
    BinaryOp, UnaryOp, CompareOp, CallingConvention, Linkage,
    FunctionSignatureBuilder, IrValue, InlineHint,
};
use crate::tast::{
    node::{TypedFile, TypedFunction, TypedStatement, TypedExpression, TypedExpressionKind,
           TypedParameter, FunctionEffects, BinaryOperator, UnaryOperator,
           TypedCatchClause, TypedSwitchCase, HasSourceLocation, LiteralValue},
    Type, TypeKind, TypeId,
    SymbolId, InternedString, SourceLocation, Visibility,
    SymbolTable,
    TypeTable,
};
use crate::semantic_graph::{
    SemanticGraphs, analysis::analysis_engine::{AnalysisResults, HIRLoweringHints},
};
...
impl<'a> LoweringContext<'a> {
    ...
    fn lower_statement(&mut self, statement: &TypedStatement) {
        ...
        match statement {
            ...
            TypedStatement::VarDeclaration { symbol_id, initializer, .. } => {
                ...
                let var_reg = self.builder.declare_local(
                    format!("var_{}", symbol_id.as_raw()),
                    var_type.clone()
                );
            ...
            TypedStatement::Switch { discriminant, cases, default_case, .. } => {
                ...
                let default_stmts = default_case.as_ref().map(|stmt| {
            ...
            TypedStatement::Try { body, catch_clauses, finally_block, .. } => {
                ...
                self.add_error("Try statement lowering not implemented", statement.source_location());
            }
    ...
    fn lower_expression(&mut self, expr: &TypedExpression) -> Option<IrId> {
        ...
        match &expr.kind {
            ...
            EK::Variable { symbol_id } => {
                self.symbol_map.get(symbol_id).copied()
            }
            
            EK::BinaryOp { left, operator, right } => {
                self.lower_binary_op(operator, left, right)
            }
            
            EK::UnaryOp { operator, operand } => {
                self.lower_unary_op(operator, operand)
            }
            ...
            EK::FunctionCall { function, arguments, .. } => {
                self.lower_call(function, arguments, expr.expr_type)
            }
            
            EK::FieldAccess { object, field_symbol, .. } => {
            ...
            EK::ArrayAccess { array, index } => {
                self.lower_array_access(array, index)
            }
            
            EK::ArrayLiteral { elements } => {
                self.lower_array_literal(elements, expr.expr_type)
            }
            
            EK::ObjectLiteral { fields, .. } => {
                // Convert TypedObjectField to expected format
                let converted_fields: Vec<(String, TypedExpression)> = fields.iter()
                    .map(|f| (f.name.to_string(), f.value.clone()))
                    .collect();
                self.lower_object_literal(&converted_fields, expr.expr_type)
            }
            
            EK::Cast { expression, target_type, .. } => {
    ...
    fn lower_binary_op(
        ...
        match op {
            ...
            BinaryOperator::Mod => {
                let op = if is_float { BinaryOp::FRem } else { BinaryOp::Rem };
                self.builder.build_binop(op, left_val, right_val)
            }
            BinaryOperator::Eq => {
                let op = if is_float { CompareOp::FEq } else { CompareOp::Eq };
                self.builder.build_cmp(op, left_val, right_val)
            }
            BinaryOperator::Ne => {
                let op = if is_float { CompareOp::FNe } else { CompareOp::Ne };
                self.builder.build_cmp(op, left_val, right_val)
            }
            BinaryOperator::Lt => {
                let op = if is_float { CompareOp::FLt } else { CompareOp::Lt };
                self.builder.build_cmp(op, left_val, right_val)
            }
            BinaryOperator::Le => {
                let op = if is_float { CompareOp::FLe } else { CompareOp::Le };
                self.builder.build_cmp(op, left_val, right_val)
            }
            BinaryOperator::Gt => {
                let op = if is_float { CompareOp::FGt } else { CompareOp::Gt };
                self.builder.build_cmp(op, left_val, right_val)
            }
            BinaryOperator::Ge => {
                let op = if is_float { CompareOp::FGe } else { CompareOp::Ge };
                self.builder.build_cmp(op, left_val, right_val)
            }
    ...
    fn lower_logical_and(
        ...
        self.builder.build_cond_branch(left_val, right_block, merge_block)?;
        ...
        self.builder.add_phi_incoming(merge_block, phi, current_block, false_val)?;
        self.builder.add_phi_incoming(merge_block, phi, right_block, right_val)?;
    ...
    fn lower_logical_or(
        ...
        // Branch based on left value
        self.builder.build_cond_branch(left_val, merge_block, right_block)?;
        ...
        self.builder.add_phi_incoming(merge_block, phi, current_block, true_val)?;
        self.builder.add_phi_incoming(merge_block, phi, right_block, right_val)?;
    ...
    fn lower_variable_declaration(
        ...
        self.add_error("Variable declaration lowering not implemented", SourceLocation::unknown());
    }
    ...
    fn lower_while_loop(&mut self, condition: &TypedExpression, body: &TypedStatement) {
        ...
        if let Some(cond_val) = self.lower_expression(condition) {
            self.builder.build_cond_branch(cond_val, body_block, exit_block);
        }
    ...
    fn lower_switch_statement(
        ...
        self.add_error("Switch statement lowering not implemented", SourceLocation::unknown());
    }
    ...
    fn lower_try_statement(
        ...
        let Some(try_bb) = self.builder.create_block_with_label("try".to_string()) else { return; };
        let Some(catch_bb) = self.builder.create_block_with_label("catch".to_string()) else { return; };
        let Some(finally_bb) = self.builder.create_block_with_label("finally".to_string()) else { return; };
        let Some(continue_bb) = self.builder.create_block_with_label("continue".to_string()) else { return; };
        ...
        for catch in catches {
            ...
            let Some(this_catch_bb) = self.builder.create_block_with_label("catch_clause".to_string()) else { continue; };
    ...
    fn lower_call(
        ...
        for arg in arguments {
            if let Some(arg_reg) = self.lower_expression(arg) {
                ...
                self.add_error(
                    "Failed to lower function argument",
                    arg.source_location,
                );
                return None;
            }
